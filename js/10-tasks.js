...
(function () {
    ...
    function formatDueLabel(due) {
        ...
        if (diffDays < 0)
            return { text: 'Overdue · ' + dateStr, overdue: true };
        if (diffDays === 0)
    ...
    function updateBadgeAndSub(list) {
        ...
        if (sub) {
            if (list.length === 0)
                ...
                sub.textContent = incomplete === 0 ? 'All done! 🎉' : (incomplete + ' task' + (incomplete === 1 ? '' : 's') + ' left');
        }
    }
    // ── RENDER LIST ──
    function renderList() {
        ...
        if (typeof window._wireTaskSwipe === 'function')
            window._wireTaskSwipe();
    }
    // ── VIEW SWITCHING ──
    function showList() {
        currentId = null;
    ...
    window.taskOpen = function (id) {
        ...
        showEditor();
        setTimeout(function () { if (!task.title && ti)
            ti.focus(); }, 320);
    };
    ...
    window.taskConfirmClose = function (e) {
        if (e && e.target !== document.getElementById('tasksConfirmOverlay'))
            return;
        document.getElementById('tasksConfirmOverlay').classList.remove('open');
    };
...
(function () {
    ...
    function onDown(e) {
        ...
        if (!fg || e.target.closest('.task-check'))
            return;
    ...
    function onMove(e) {
        ...
        if (sw.locked === null) {
            ...
            if (sw.locked) {
                try {
                    sw.fg.setPointerCapture(sw.pointerId);
                }
                catch (err) { }
            }
        }
        ...
        if (bg) {
            if (clamped < -10)
                bg.innerHTML = '<span class="tsw-label tsw-right">🗑️ Delete</span>';
            else if (clamped > 10)
                bg.innerHTML = '<span class="tsw-label tsw-left">✅ Done</span>';
            else
                bg.innerHTML = '';
        }
    }
    function onUp(e) {
        ...
        if (sw.locked && dx <= -THRESH) {
            ...
            setTimeout(function () { if (typeof window.taskDeleteSwipe === 'function')
                window.taskDeleteSwipe(id); }, 160);
        }
        else if (sw.locked && dx >= THRESH) {
            fg.style.transform = 'translateX(0)';
            if (bg)
                bg.innerHTML = '';
            if (typeof window.taskToggleDone === 'function')
                window.taskToggleDone(id);
        }
        else {
            fg.style.transform = 'translateX(0)';
            if (bg)
...
window.ntAddCurrent = function () {
    if (typeof window.notesCreate === 'function')
        window.notesCreate();
};
