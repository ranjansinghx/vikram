...
function bdtLoad() { try {
    return JSON.parse(localStorage.getItem(BDT_KEY) || '[]');
}
catch (e) {
    return [];
} }
...
function bdtRefreshDays() {
    ...
    let maxD = 32;
    try {
        if (window.BS_DATA && BS_DATA[y])
            maxD = BS_DATA[y][m - 1] || 32;
    }
    catch (e) { }
...
function bdtOpenAdd() {
    ...
    if (svEl)
        svEl.style.pointerEvents = 'none';
    // Scroll to top so Save button is always reachable
    setTimeout(() => { const sc = document.querySelector('#bdtSheet .sscroll'); if (sc)
        sc.scrollTop = 0; }, 80);
}
...
function bdtSyncToEvents() {
    ...
    for (let i = userEvents.length - 1; i >= 0; i--) {
        if (userEvents[i].isBdtBirthday)
            userEvents.splice(i, 1);
    }
    const list = bdtLoad();
    list.forEach(item => {
        if (!item.y || !item.m || !item.d || !item.name)
            return;
        try {
    ...
    if (typeof renderSelectedDay === 'function')
        renderSelectedDay();
    if (typeof renderScheduleView === 'function' && typeof scheduleActive !== 'undefined' && scheduleActive)
        renderScheduleView();
...
function bdtDaysUntil(bY, bM, bD) {
    try {
        ...
        if (next < today)
            next = new Date(thisYear + 1, ad.getMonth(), ad.getDate());
...
function bdtRender() {
    ...
    const list = bdtLoad();
    if (subEl)
        subEl.textContent = list.length === 0 ? 'No birthdays saved yet' : `${list.length} birthday${list.length !== 1 ? 's' : ''} saved`;
    if (list.length === 0) {
        el.innerHTML = '';
        return;
    }
...
(function () {
    ...
    window.switchTab = function (tab) {
        if (typeof _baseBdt === 'function')
            _baseBdt.apply(this, arguments);
        if (tab === 'settings')
            setTimeout(bdtRender, 100);
    };
})();
// ══════════════════════════════════════════════════════════════════════════════
...
function simpleModeExpandMonth() {
    try {
        ...
        [50, 150, 350, 700].forEach(function (delay) {
            setTimeout(function () {
                if (cfg.simpleMode && typeof applyCalHeight === 'function')
...
function simpleModeJustEnabled() {
    if (!cfg.simpleMode)
        return;
    try {
        if (typeof dayViewActive !== 'undefined' && dayViewActive && typeof toggleDayView === 'function')
            toggleDayView();
        if (typeof weekViewActive !== 'undefined' && weekViewActive && typeof toggleWeekView === 'function')
            toggleWeekView();
        if (typeof monthTabActive !== 'undefined' && monthTabActive && typeof toggleMonthTab === 'function')
            toggleMonthTab();
        if (typeof scheduleActive !== 'undefined' && scheduleActive && typeof toggleScheduleView === 'function')
            toggleScheduleView();
    }
    catch (e) { }
    try {
...
function applyAccentColor(name) {
    ...
    if (sub)
        sub.textContent = 'Currently ' + ac.label;
    const psub = document.getElementById('profAccentSub');
    if (psub)
        psub.textContent = ac.label;
}
...
function closeYearInReview(e) {
    if (e && e.target !== document.getElementById('yirOverlay'))
        return;
    const overlay = document.getElementById('yirOverlay');
...
function renderYearInReview(bsYear) {
    ...
    if (!months) {
        body.innerHTML = '<p style="color:var(--dsub)">Year data not available</p>';
        return;
    }
    ...
    for (let m = 1; m <= 12; m++) {
        ...
        for (let d = 1; d <= days; d++) {
            ...
            let bg = 'var(--tgbg)';
            if (hasHol)
                bg = 'rgba(239,143,149,.7)';
            else if (hasBd)
                bg = 'rgba(236,72,153,.55)';
            else if (hasEvt)
                bg = 'var(--accent)';
...
function applyDefReminder() {
    ...
    if (sub)
        sub.textContent = 'Default: ' + (labels[cfg.defReminder] || cfg.defReminder + ' min') + ' before event';
...
function applyCountBadge() {
    ...
    if (el)
        el.style.display = cfg.showCountBadge ? '' : 'none';
...
async function _fbDeriveKey(password, saltBytes) {
    ...
    return crypto.subtle.deriveKey({ name: 'PBKDF2', salt: saltBytes, iterations: 150000, hash: 'SHA-256' }, keyMaterial, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
function _fbBufToB64(buf) { return btoa(String.fromCharCode(...new Uint8Array(buf))); }
function _fbB64ToBuf(b64) { const bin = atob(b64); const arr = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++)
    arr[i] = bin.charCodeAt(i); return arr.buffer; }
async function doExportFullBackup() {
    ...
    if (encrypt) {
        if (!pw || pw.length < 4) {
            if (typeof vikramToast === 'function')
                vikramToast('⚠️ Password must be at least 4 characters');
            return;
        }
        if (!_fbCryptoAvailable()) {
            if (typeof vikramToast === 'function')
                vikramToast('❌ Encryption not available on this device');
            return;
        }
        try {
    ...
    a.href = url;
    a.download = `vikram-backup-${ymd}.json`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 2000);
    if (typeof vikramToast === 'function')
        vikramToast('✅ Backup downloaded');
}
async function importFullBackup(input) {
    ...
    try {
        ...
        if (typeof vikramToast === 'function')
            vikramToast('❌ Could not read file');
        return;
    }
    let payload;
    try {
        ...
        if (typeof vikramToast === 'function')
            vikramToast('❌ Invalid file format');
        return;
    }
    if (!payload || (payload.version !== 'vikram-full-1')) {
        if (typeof vikramToast === 'function')
            vikramToast('❌ Not a Vikram full-backup file');
        return;
    }
    let data;
    if (payload.encrypted) {
        ...
        if (!pw) {
            ...
            if (typeof vikramToast === 'function')
                vikramToast('🔒 Enter the backup password first');
            return;
        }
        try {
            ...
            if (typeof vikramToast === 'function')
                vikramToast('❌ Wrong password or corrupted file');
            return;
        }
    }
    else {
    ...
    svShowConfirm('🗄️', 'Restore Full Backup', `This will overwrite your current events, notes, tasks, habits, and cycle data with the contents of this file (from ${new Date(payload.exportedAt).toLocaleDateString()}). This can't be undone.`, 'Restore', '#6366f1', function () {
        try {
            Object.keys(data).forEach(k => {
                if (FULL_BACKUP_KEYS.includes(k))
                    localStorage.setItem(k, data[k]);
            });
        }
        catch (e) {
            if (typeof vikramToast === 'function')
                vikramToast('❌ Restore failed — storage may be full');
            return;
        }
        if (typeof vikramToast === 'function')
            vikramToast('✅ Backup restored — reloading…');
...
function undoSnackbarAction() {
    ...
    if (_undoState && typeof _undoState.restoreFn === 'function') {
        try {
            ...
            console.error('[Vikram] undo failed:', e);
        }
    }
    clearTimeout(_undoTimer);
    hideUndoSnackbar();
...
function parseQuickAddText(text) {
    ...
    if (/\bday after tomorrow\b/i.test(title)) {
        ...
        if (wdMatch) {
            ...
            if (diff === 0)
                diff = 7; // same weekday as today means the one next week
...
function quickAddParseAndFill() {
    ...
    if (parsed.timeStr) {
        ...
        if (st) {
            ...
            if (typeof onEvTimeChange === 'function')
                onEvTimeChange();
        }
    }
    input.value = '';
    if (typeof vikramToast === 'function')
        vikramToast('✨ Filled in — check the details below');
    if (titleEl)
        titleEl.focus();
}
// ══════════════════════════════════════════════════════════════════════════════
    ...
    try {
        ...
        setTimeout(function () {
            if (action === 'addEvent' && typeof openAddForDay === 'function' && typeof TODAYBS !== 'undefined') {
                ...
                if (typeof switchTab === 'function')
                    switchTab('cycle');
                setTimeout(function () { if (typeof cyLogPeriodStart === 'function')
                    cyLogPeriodStart(); }, 350);
            }
...
function _statsGather() {
    ...
    try {
        notes = JSON.parse(localStorage.getItem('vikram_notes_v2') || '[]');
    }
    catch (e) { }
    try {
    ...
    try {
        ...
        if (first)
            memberDays = Math.max(1, Math.floor((Date.now() - new Date(first).getTime()) / 86400000) + 1);
    }
    catch (e) { }
    return {
...
function renderProfileExtras() {
    ...
    try {
        renderProfileDataSummary();
    }
    catch (e) { }
    try {
        let p = {};
        try {
            p = JSON.parse(localStorage.getItem('vikram_profile') || '{}');
        }
        catch (e) { }
        if (typeof renderProfileQR === 'function')
            renderProfileQR('https://vikram-nep.pages.dev/');
        if (typeof cfg !== 'undefined' && cfg.accentColor && typeof applyAccentColor === 'function')
...
function initAllSettingsFeatures() {
    ...
    if (yirLabel)
        yirLabel.textContent = String(vY);
}
// Run on page load
...
window.switchTab = function (tab) {
    ...
    if (tab !== 'settings')
        applyPinnedNote(); // refresh pinned note whenever leaving settings
};
