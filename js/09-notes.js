...
(function () {
    ...
    function renderList() {
        ...
        if (list.length === 0) {
            if (empty) {
                ...
                if (sub)
                    sub.textContent = _activeTagFilter ? 'No notes tagged #' + _activeTagFilter + '.' : 'Tap + New Note to write something.';
            }
            container.innerHTML = '';
            return;
        }
    ...
    window.notesConfirmClose = function (e) {
        if (e && e.target !== document.getElementById('notesConfirmOverlay'))
            return;
        document.getElementById('notesConfirmOverlay').classList.remove('open');
    };
    // ── SAVE CURRENT NOTE ──
