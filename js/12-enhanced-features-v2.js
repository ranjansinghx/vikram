...
window.renderSelectedDay = function () {
    if (typeof _origRenderSelectedDay === 'function')
        _origRenderSelectedDay();
...
window.openSearch = function () {
    ...
    if (jumpY && typeof vY !== 'undefined')
        jumpY.value = vY;
    const jumpM = document.getElementById('jumpM');
    if (jumpM && typeof vM !== 'undefined')
        jumpM.value = vM;
    setTimeout(() => {
...
window.searchRun = function () {
    ...
    results.innerHTML = hits.map(h => {
        let onclick;
        if (h.type === 'hol' || h.type === 'evt')
            onclick = `searchGoTo(${h.ky},${h.km},${h.kd})`;
        else if (h.type === 'note')
            onclick = `searchOpenNote('${h.id}')`;
        else if (h.type === 'task')
            onclick = `searchOpenTask('${h.id}')`;
        else if (h.type === 'habit')
            onclick = `searchOpenHabit(${h.id})`;
        else if (h.type === 'kharcha')
            onclick = `searchOpenKharcha('${h.id}')`;
        else if (h.type === 'cycle')
            onclick = `searchOpenCycleLog(${h.date})`;
        else
            onclick = '';
        return `
...
window.searchOpenNote = function (id) {
    ...
    if (typeof haptic === 'function')
        haptic('medium');
    if (typeof window.profileOpenNote === 'function')
        window.profileOpenNote(id);
};
window.searchOpenTask = function (id) {
    document.getElementById('searchModal')?.classList.remove('open');
    if (typeof haptic === 'function')
        haptic('medium');
    if (typeof window.profileOpenTask === 'function')
        window.profileOpenTask(id);
};
window.searchOpenHabit = function (id) {
    ...
    if (typeof switchTab === 'function')
        switchTab('counter');
    setTimeout(function () { if (typeof window.dcOpenEdit === 'function')
        window.dcOpenEdit(id); }, 300);
};
...
window.searchOpenCycleLog = function (dateTs) {
    ...
    if (typeof switchTab === 'function')
        switchTab('cycle');
    setTimeout(function () { if (typeof openSymptomLogSheet === 'function')
        openSymptomLogSheet(dateTs); }, 300);
};
...
window.openShareModal = function () {
    ...
    if (typeof HOL !== 'undefined') {
        ...
        if (HOL[hkey])
            items.push({ icon: '🇳🇵', name: HOL[hkey].en });
    }
    if (typeof OBS !== 'undefined') {
        const okey = `${bsY}-${bsM}-${bsD}`;
        if (OBS[okey])
            items.push({ icon: '🎀', name: OBS[okey].en });
    }
    if (typeof userEvents !== 'undefined') {
...
window.doShare = function () {
    haptic('medium');
    const sd = typeof selectedDay !== 'undefined' ? selectedDay : (typeof TODAYBS !== 'undefined' ? TODAYBS : {});
    const bsM = sd.m || 1;
    const monthName = typeof MEN !== 'undefined' ? MEN[bsM - 1] : '';
...
window.notesFmt = function (cmd, val) {
    ...
    if (typeof _origNotesFmt === 'function')
        _origNotesFmt(cmd, val);
};
    ...
    try {
        ...
        if (p.bday && p.bday.y) {
            setTimeout(function () {
                ...
                if (disp)
                    disp.textContent = typeof bdayDisplayText === 'function' ? bdayDisplayText(p.bday) : '';
            }, 300);
        }
...
function getShareableEvents() {
    if (typeof userEvents === 'undefined' || !userEvents.length)
        return [];
...
function checkIncomingSharedProfile() {
    try {
        ...
        if (av)
            av.textContent = profileInitials(pn);
        const nd = document.getElementById('sharedProfileName');
        if (nd)
            nd.textContent = pn || (lang === 'ne' ? 'कसैले' : 'Someone');
...
function renderProfileShareEvents(events) {
    ...
    const upcoming = (typeof userEvents === 'undefined' ? [] : userEvents)
        .map((ev, i) => ({ ev, i }))
        .filter(({ ev }) => {
        if (ev.isBirthday || ev.isBdtBirthday)
            return false; // birthdays shown separately
...
function renderProfileBirthdays() {
    ...
    list.forEach(item => { if (item.y && item.m && item.d && item.name)
        allBdays.push({ ...item, isProfile: false }); });
    if (!allBdays.length) {
        wrap.innerHTML = '';
        if (empty)
            empty.style.display = '';
    ...
    wrap.innerHTML = shown.map(item => {
        ...
        let badge = '';
        if (item.days === 0)
            badge = `<span style="background:linear-gradient(135deg,#ec4899,#f43f5e);color:#fff;font-size:10px;font-weight:800;padding:2px 7px;border-radius:20px;flex-shrink:0;">🎉 Today!</span>`;
        else if (item.days !== null && item.days <= 7)
            badge = `<span style="background:rgba(236,72,153,.15);color:#ec4899;font-size:10px;font-weight:800;padding:2px 7px;border-radius:20px;flex-shrink:0;">in ${item.days}d</span>`;
        else if (item.days !== null)
            badge = `<span style="background:var(--tgbg);color:var(--dsub);font-size:10px;font-weight:700;padding:2px 7px;border-radius:20px;flex-shrink:0;">${item.days}d</span>`;
...
function renderProfileNotes() {
    ...
    let list = [];
    try {
        list = JSON.parse(localStorage.getItem('vikram_notes_v2') || '[]') || [];
    }
    catch (e) {
        list = [];
    }
...
function renderProfileTasks() {
    ...
    let list = [];
    try {
        list = JSON.parse(localStorage.getItem('vikram_tasks_v1') || '[]') || [];
    }
    catch (e) {
        list = [];
    }
    ...
    list.sort((a, b) => {
        ...
        if (!a.done) {
            if (a.due && b.due)
                return a.due < b.due ? -1 : (a.due > b.due ? 1 : (b.updated || 0) - (a.updated || 0));
            if (a.due)
                return -1;
            if (b.due)
                return 1;
...
function renderProfileBackup() {
    ...
    if (!signedOutEl || !signedInEl || !listEl)
        return;
...
window.profileOpenNote = function (id) {
    ...
    closeAll();
    switchTab('notes');
    setTimeout(function () { if (typeof window.notesOpen === 'function')
        window.notesOpen(id); }, 300);
};
window.profileOpenTask = function (id) {
    ...
    switchTab('home');
    setUpView('tasks');
    setTimeout(function () { if (typeof window.taskOpen === 'function')
        window.taskOpen(id); }, 300);
};
window.profileEditEvent = function (i) {
    ...
    setTimeout(() => {
        ...
        if (picker) {
            ...
            if (target && typeof selectEvCat === 'function')
                selectEvCat(target);
        }
...
window.profileDeleteEvent = function (adMs) {
    ...
    if (wasBirthday) {
        try {
            ...
            const disp = document.getElementById('profileBdayDisplay');
            if (disp)
                disp.textContent = bdayDisplayText(null);
        }
        catch (e) { }
    }
    render();
...
function profileBdayCardUpdate(bday) {
    ...
    if (dateEl)
        dateEl.textContent = `${MEN[bday.m - 1]} ${bday.d}, ${bday.y} BS`;
...
function bdayYearOpts(sel) {
    ...
    for (let y = maxY; y >= minY; y--)
        h += `<option value="${y}"${y === sel ? ' selected' : ''}>${y}</option>`;
    return h;
}
...
window.profileBdayStartEdit = function () {
    ...
    const dEl = document.getElementById('profileBdayD');
    if (yEl)
        yEl.innerHTML = bdayYearOpts(defY);
    if (mEl)
        mEl.innerHTML = bdayMonthOpts(defM);
    if (dEl)
        dEl.innerHTML = bdayDayOpts(defY, defM, defD);
    const rmBtn = document.getElementById('profileBdayRemoveBtn');
    if (rmBtn)
        rmBtn.style.display = b.y ? 'block' : 'none';
    const dispRow = document.getElementById('profileBdayRow');
...
window.profileSaveBday = function () {
    haptic('medium');
    const y = +(document.getElementById('profileBdayY') || {}).value;
    const m = +(document.getElementById('profileBdayM') || {}).value;
    const d = +(document.getElementById('profileBdayD') || {}).value;
    if (!y || !m || !d)
        return;
    let p = {};
    try {
    ...
    const disp = document.getElementById('profileBdayDisplay');
    if (disp)
        disp.textContent = bdayDisplayText(p.bday);
    const editRow = document.getElementById('profileBdayEditRow');
    if (editRow)
        editRow.style.display = 'none';
    profileBdayCardUpdate(p.bday);
