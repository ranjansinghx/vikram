...
(function () {
        ...
        try {
            ...
            if (s && typeof s === 'object') {
                if (typeof s.enabled === 'boolean')
                    lockState.enabled = s.enabled;
                if (typeof s.scope === 'string')
                    lockState.scope = s.scope;
                if (typeof s.salt === 'string')
                    lockState.salt = s.salt;
                if (typeof s.hash === 'string')
                    lockState.hash = s.hash;
                if (typeof s.bioEnabled === 'boolean')
                    lockState.bioEnabled = s.bioEnabled;
                if (typeof s.credId === 'string')
                    lockState.credId = s.credId;
            }
        }
        catch (e) { }
    }
    function lockSaveState() {
        try {
            localStorage.setItem('vikram_lock_v1', JSON.stringify(lockState));
        }
        catch (e) { }
    }
    lockLoadState();
    ...
    window.lockKeyPress = function (d) {
        ...
        if (_lockEnteredPin.length === 4)
            setTimeout(lockVerifyEntered, 120);
    };
    ...
    function lockUnlockSuccess() {
        ...
        const cb = _lockPendingUnlockCallback;
    ...
    function lockShowAppScreen(cb) {
        ...
        if (subEl)
            subEl.textContent = lockState.scope === 'notes' ? 'Notes are locked' : 'Vikram is locked';
        const bioBtn = document.getElementById('lockBioBtn');
        if (bioBtn)
            bioBtn.style.display = (lockState.bioEnabled && lockBiometricAvailable()) ? 'flex' : 'none';
        _lockEnteredPin = '';
        lockUpdateDots('', 'lockDots');
        const err = document.getElementById('lockError');
        if (err)
    ...
    window.lockTryBiometric = async function () {
        if (!lockState.bioEnabled || !lockState.credId || !lockBiometricAvailable())
            return;
        try {
            const challenge = crypto.getRandomValues(new Uint8Array(32));
    ...
    function lockRefreshSettingsUI() {
        ...
        if (scopeRow)
            scopeRow.style.display = lockState.enabled ? '' : 'none';
        if (pinRow)
            pinRow.style.display = lockState.enabled ? '' : 'none';
        if (bioRow)
            bioRow.style.display = (lockState.enabled && hasPin && lockBiometricAvailable()) ? '' : 'none';
        ...
        if (pinLbl)
            pinLbl.textContent = hasPin ? 'Change PIN' : 'Set PIN';
        const sub = document.getElementById('lockEnabledSub');
        if (sub)
            sub.textContent = lockState.scope === 'notes' ? 'Require a PIN to open Notes' : 'Require a PIN to open Vikram';
    ...
    window.lockToggleEnabled = function (btn) {
        ...
        if (!lockCryptoAvailable()) {
            alert('App Lock needs a secure (https) connection and is not available here.');
            return;
        }
    ...
    window.lockToggleBiometric = function (btn) {
        ...
        if (!lockState.bioEnabled) {
            lockRegisterBiometric().then(ok => {
                if (!ok)
                    alert("Couldn't set up biometric unlock on this device.");
                lockRefreshSettingsUI();
            });
        }
        else {
    ...
    window.lockOpenSetupSheet = function (onComplete) {
        if (!lockCryptoAvailable()) {
            alert('Setting a PIN needs a secure (https) connection and is not available here.');
            return;
        }
        ...
        _lockSetupPin = '';
        _lockSetupFirstPin = '';
        const t = document.getElementById('lockSetupTitle');
        if (t)
            t.textContent = lockState.hash ? '🔢 Change PIN' : '🔢 Set a PIN';
        const s = document.getElementById('lockSetupSub');
        if (s)
            s.textContent = 'Enter a new 4-digit PIN';
        const e = document.getElementById('lockSetupError');
        if (e)
            e.textContent = '';
        ...
        if (ov) {
            ...
            ov.dataset.sheetOrigin = 'lockSetupSheet';
        }
    };
    ...
    window.lockSetupKeyPress = function (d) {
        ...
        if (_lockSetupPin.length === 4)
            setTimeout(lockSetupAdvance, 120);
    };
    ...
    async function lockSetupAdvance() {
        if (_lockSetupStage === 'new') {
            ...
            const s = document.getElementById('lockSetupSub');
            if (s)
                s.textContent = 'Confirm your PIN';
            lockUpdateDots('', 'lockSetupDots');
        }
        else {
            if (_lockSetupPin === _lockSetupFirstPin) {
                ...
                const e = document.getElementById('lockSetupError');
                if (e)
                    e.textContent = "PINs didn't match — try again";
                if (typeof haptic === 'function')
                    haptic('heavy');
        ...
        window.switchTab = function (tab) {
            if (tab === 'notes' && lockState.enabled && lockState.scope === 'notes' && lockState.hash && !_lockSessionUnlocked) {
                lockShowAppScreen(function () {
                    if (typeof _baseForLock === 'function')
                        _baseForLock.call(this, tab);
                });
                return;
            }
            if (typeof _baseForLock === 'function')
                _baseForLock.apply(this, arguments);
        };
    })();
    ...
    document.addEventListener('visibilitychange', function () {
        if (!lockState.enabled || !lockState.hash)
            return;
        if (document.hidden) {
