(function(){
  ...
  function shouldShow(){
    ...
    try{ if(localStorage.getItem(INSTALLED_KEY) === '1') return false; }catch(e){}
    return true;
  }
  ...
  function openCard(){
    ...
    if(isIOS){
      ...
      if(btn) btn.style.display = 'none'; // Native install not available on iOS
    }
  ...
  document.getElementById('pwaInstallOverlay').addEventListener('click', function(e){
    if(e.target === this) window.pwaDismiss();
  });
