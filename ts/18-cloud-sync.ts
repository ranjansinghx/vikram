...
(function(){
  ...
  function applyCloudPayload(cloud){
    ...
    SYNC_KEYS.forEach(function(sk){
      ...
      try{
        ...
        if(meta) localTs = JSON.parse(meta).ts || 0;
      }catch(e){}
    ...
    if(changed){
      ...
      _reloadRuntimeState();
      if(typeof vikramToast === 'function') vikramToast('☁️ Data synced from cloud');
    }
    _isMerging = false;
  }
  ...
  function _reloadRuntimeState(){
    ...
    try{
      ...
      if(typeof cfg !== 'undefined') Object.assign(cfg, c);
    }catch(e){}
    // Cycle
    try{
    ...
    try{
      if(typeof window.renderKharchaSheet === 'function') window.renderKharchaSheet();
    }catch(e){}
    ...
    try{
      if(typeof famLocal === 'function' && typeof famEnsureAuth === 'function' && typeof famAttachListener === 'function'){
        ...
        if(famNow && famNow.code && window._famLastSyncedCode !== famNow.code){
          ...
          famEnsureAuth(function(ok){ if(ok) famAttachListener(famNow.code); });
        }
        if(typeof window.renderFamilySheet === 'function') window.renderFamilySheet();
      }
    }catch(e){}
    // Re-render
    try{
      ...
      if(typeof bdtSyncToEvents === 'function') bdtSyncToEvents();
      if(typeof window.dcRender === 'function') window.dcRender();
      if(typeof window.whRenderCard === 'function') window.whRenderCard();
    }catch(e){}
  }

  // ── Push local data to cloud (debounced) ────────────────────────────────────
  function schedulePush(){
    if(!_uid || _isMerging) return;
  ...
  function doPush(){
    ...
    SYNC_KEYS.forEach(function(sk){
      if(payload[sk.key]){
        ...
        var meta = null;
        try{ meta = JSON.parse(localStorage.getItem(sk.key + '__syncmeta') || 'null'); }catch(e){}
        payload[sk.key].ts = meta ? Math.max(meta.ts, Date.now()) : Date.now();
        // Update syncmeta
        try{ localStorage.setItem(sk.key + '__syncmeta', JSON.stringify({ ts: payload[sk.key].ts })); }catch(e){}
      }
    });
  ...
  window.vikramCloudSync = {
    startListening: function(uid){
      ...
      if(!ref){
        ...
        setTimeout(function(){ if(_uid) window.vikramCloudSync.startListening(_uid); }, 1000);
        return;
      }
    ...
    stopListening: function(){
      if(_listener && _dbRef){
        try{ _dbRef.off('value', _listener); }catch(e){}
      }
      _listener = null;
      _uid = null;
      _updateSyncUI(false);
  ...
  window.saveEvents = function(){
    if(typeof _origSaveEvents === 'function') _origSaveEvents.apply(this, arguments);
    if(window.vikramCloudSync) window.vikramCloudSync.push();
  };
  // Patch saveCycle
  var _origSaveCycle = window.saveCycle;
  window.saveCycle = function(){
    if(typeof _origSaveCycle === 'function') _origSaveCycle.apply(this, arguments);
    if(window.vikramCloudSync) window.vikramCloudSync.push();
  };
