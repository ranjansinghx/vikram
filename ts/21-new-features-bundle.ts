...
function T(msg, dur?){ if(typeof window.vikramToast==='function') window.vikramToast(msg, dur); }
function H(type){ if(typeof window.haptic==='function') window.haptic(type||'light'); }
...
window.renderKharchaSheet = function(){
  ...
  if(!cats.find(c=>c[0]===st.category)) st.category = cats[0][0];
...
window.kharchaAddEntry = function(){
  ...
  if(!amount || amount<=0){ T('⚠️ Enter a valid amount'); return; }
  const list = khLoad();
  list.push({
    id: uid('kh'),
...
window.alarmOpenNew = function(){
  ...
  setTimeout(()=>{
    ...
    if(t){ const now=new Date(); t.value = String(now.getHours()).padStart(2,'0')+':'+String(now.getMinutes()).padStart(2,'0'); }
  }, 30);
};
...
window.alarmToggleWd = function(d){
  ...
  if(idx>=0) window._alDraftDays.splice(idx,1); else window._alDraftDays.push(d);
  window.renderAlarmsSheet();
};
window.alarmSaveNew = async function(){
  ...
  if(typeof window.requestNotifPerm === 'function'){
    try{ await window.requestNotifPerm(); }catch(e){}
  }
  H('success');
  T('⏰ Alarm saved');
  window._alFormOpen = false;
...
function alCheckDue(){
  try{
    ...
    list.forEach(a=>{
      if(!a.enabled) return;
      if(a.hour!==now.getHours() || a.minute!==now.getMinutes()) return;
      if(a.firedFor === todayKey) return;
      if(a.days.length>0 && !a.days.includes(now.getDay())) return;
      // Fire!
      const title = '⏰ ' + (a.label || 'Alarm');
      const body = alFmtTime(a.hour,a.minute);
      if(typeof window.logNotifHistory === 'function') window.logNotifHistory(title, body, 'alarm');
      try{
      ...
      H('medium');
      a.firedFor = todayKey;
      if(a.days.length===0) a.enabled = false; // one-time alarms auto-disable
      changed = true;
    });
    if(changed) alSave(list);
  }catch(e){}
}
...
window.renderInsightsSheet = function(){
  ...
  `;
  if(tab==='fx') insightsLoadGoldFx();
  else if(tab==='rashifal') window.renderRashifalSheet();
  else insightsRenderMuhurat();
};
...
function insightsLoadWeather(targetId){
  ...
  if(fresh){ insightsRenderWeather(cache.data, !!cache.approx, targetId); return; }
...
function insightsLoadGoldFx(){
  ...
  if(fresh){ insightsRenderGoldFx(cache.data); return; }
...
window.insightsRetryGoldFx = function(){
  const pane = document.getElementById('insightsPane');
  if(pane) pane.innerHTML = window.skelRowsHTML(4, {header:true});
...
async function insightsFetchGoldFx(staleCache){
  ...
  try{
    ...
    if(res.ok){
      ...
      if(goldM && parseFloat(goldM[1].replace(/,/g,'')) > 0) result.gold = goldM[1];
      if(silverM && parseFloat(silverM[1].replace(/,/g,'')) > 0) result.silver = silverM[1];
    }
    ...
    if(res2.ok){
      ...
      FX_CURRENCIES.forEach(([code,label,unit])=>{
        ...
        if(m){
          ...
          const div = (pageUnit || unit || 1) as number;
  ...
  if(hasData){
    ...
    if(window._insTab==='fx') insightsRenderGoldFx(result);
...
function insightsRenderGoldFx(data, staleTs?){
  ...
  if(hasMetal){
    ...
    if(data.gold) html += `<div class="ins-fx-row"><div class="ins-fx-name">🟡 Gold (Hallmark)</div><div class="ins-fx-val">Rs ${esc(data.gold)}</div></div>`;
    if(data.silver) html += `<div class="ins-fx-row"><div class="ins-fx-name">⚪ Silver</div><div class="ins-fx-val">Rs ${esc(data.silver)}</div></div>`;
  }
  if(hasFx){
    ...
    if(profile && profile.bday && profile.bday.y){
      ...
      for(const z of ZODIAC){
        ...
        if(sm > em){ // wraps around new year (Capricorn)
          if((mo===sm && da>=sd) || (mo===em && da<=ed)) return z.key;
        } else {
          if((mo===sm && da>=sd) || (mo===em && da<=ed) || (mo>sm && mo<em)) return z.key;
        }
      }
    }
  }catch(e){}
...
function rfSavedSign(){
  try{ return localStorage.getItem('vikram_rf_sign_choice') || null; }catch(e){ return null; }
}
...
window.renderRashifalSheet = function(){
  if(!window._rfSign) window._rfSign = rfResolveSign();
...
window.rashifalSelectSign = function(key){
  H('light');
  window._rfSign = key;
  try{ localStorage.setItem('vikram_rf_sign_choice', key); }catch(e){}
  window.renderRashifalSheet();
  if(typeof window.renderHomeRashifal === 'function') window.renderHomeRashifal();
};
...
function famEnsureAuth(cb){
  try{
    if(!window.firebase || !firebase.auth){ cb(false); return; }
    const cur = firebase.auth().currentUser;
    if(cur){ cb(true); return; }
...
window.renderFamilySheet = function(){
  ...
  if(!local.code){
    if(sub) sub.textContent = 'Shared events & shopping list';
    body.innerHTML = `
  ...
  if(sub) sub.textContent = 'Code: ' + local.code;
...
window.famCreateCode = function(){
  famEnsureAuth((ok)=>{
    if(!ok){ T('⚠️ Could not connect. Check your internet connection.'); return; }
    const code = famGenCode();
    const ref = famRef(code);
    if(!ref){ T('⚠️ Sync is unavailable right now.'); return; }
...
window.famJoinCode = function(){
  ...
  if(!code){ T('⚠️ Enter a code first'); return; }
  if(!code.startsWith('FAM')) { T('⚠️ That doesn\'t look like a Family Share code'); return; }
  famEnsureAuth((ok)=>{
    if(!ok){ T('⚠️ Could not connect. Check your internet connection.'); return; }
    const ref = famRef(code);
    if(!ref){ T('⚠️ Sync is unavailable right now.'); return; }
    ref.once('value').then(snap=>{
      if(!snap.exists()){ T('⚠️ Code not found'); return; }
...
function famAttachListener(code){
  const ref = famRef(code);
  if(!ref) return;
  if(window._famListenerRef) { try{ window._famListenerRef.off(); }catch(e){} }
  ...
  ref.on('value', snap=>{
    ...
    if(window._famConflict && window._famConflict.code === code) return;
    window._famData = { shopping: val.shopping || [], events: val.events || [] };
    window._famSyncedAt = val.updatedAt || 0;
    if(document.getElementById('familyBody')) window.renderFamilySheet();
  });
}
...
window.famConflictClose = function(e){
  if(e && e.target && e.target.id !== 'famConflictOverlay') return;
...
window.famAddShoppingItem = function(){
  ...
  if(!text){ T('⚠️ Type something to add'); return; }
...
window.famAddEvent = function(){
  ...
  if(!text){ T('⚠️ Describe the plan first'); return; }
...
window.famLeaveShare = function(){
  if(window._famListenerRef){ try{ window._famListenerRef.off(); }catch(e){} window._famListenerRef = null; }
...
(function famInit(){
  const local = famLocal();
  if(local.code){
    famEnsureAuth((ok)=>{ if(ok) famAttachListener(local.code); });
  }
})();
...
window.notesInsertChecklist = function(){
  ...
  if(typeof window.notesSave === 'function') window.notesSave();
};
...
window.noteToggleCheck = function(boxEl){
  ...
  if(textEl) textEl.classList.toggle('nc-done', checked);
  if(typeof window.notesSave === 'function') window.notesSave();
};
...
window.tplApplyTemplate = function(id){
  ...
  if(!tpl) return;
  H('light');
  if(typeof resetRecurPanel === 'function') resetRecurPanel();
  ...
  try{
    ...
    if(catBtn && typeof selectEvCat === 'function') selectEvCat(catBtn);
  }catch(e){}
  if(tpl.recur && tpl.recur.freq){
    try{
      if(typeof toggleRecurPanel === 'function') toggleRecurPanel();
      if(typeof setRecurFreq === 'function') setRecurFreq(tpl.recur.freq);
      if(tpl.recur.freq === 'weekly' && Array.isArray(tpl.recur.weekdays)){
        tpl.recur.weekdays.forEach(d=>{ if(typeof toggleRecurWd === 'function') toggleRecurWd(d); });
      }
      const extra = Math.max(0, (tpl.recur.interval||1) - 1);
      for(let i=0;i<extra;i++){ if(typeof stepRecurInterval === 'function') stepRecurInterval(1); }
    }catch(e){}
  }
...
window.tplSaveCurrentAsTemplate = function(){
  ...
  if(!title){ T('⚠️ Add a title before saving a template'); return; }
  ...
  try{
    ...
    if(onBtn && onBtn.dataset.color) category = onBtn.dataset.color;
  }catch(e){}
  let recur = null;
  try{
...
(function injectTemplatesRow(){
  function tryInject(){
    ...
    if(!addSheet) return false;
    if(document.getElementById('evTemplatesRow')) return true;
  ...
  if(typeof _origOpenSheet === 'function'){
    window.openSheet = function(t){
      _origOpenSheet(t);
      if(t === 'add'){ tryInject(); window.renderEventTemplates(); }
    };
  }
})();
...
(function highlightSearchResults(){
  ...
  if(typeof _origSearchRun !== 'function') return;
...
window.exportVikramPDF = async function(){
  ...
  catch(e){ T('⚠️ Could not load the PDF engine. Check your connection.'); return; }
  ...
  try{
    ...
    if(profile && profile.name) line('Name: ' + profile.name, 11, 7, false);
    ...
    }catch(e){ line('Could not read events.', 11, 6.5, false); }
    ...
    }catch(e){ line('No expense data yet.', 11, 6.5, false); }
...
(function homeWeatherWidget(){
  function loadIfHome(tab){
    if(tab !== 'home') return;
    if(typeof insightsLoadWeather === 'function') insightsLoadWeather('homeWeatherBody');
  }
  ...
  try{
    let startTab = 'home';
    try{ startTab = window.localStorage.getItem('vikram_active_tab') || 'home'; }catch(e){}
    loadIfHome(startTab);
  }catch(e){}
})();
...
function widgetFeedBuildPayload(){
  ...
  let tithiText: any = '';
  try{ tithiText = (typeof getTithi === 'function') ? getTithi(ad) : ''; }catch(e){}
  let sunText = '';
  try{ sunText = (typeof getPanchangText === 'function') ? getPanchangText(ad) : ''; }catch(e){}
  let nextEventTitle = '', nextEventDate = '';
  try{
    const evs = (typeof userEvents !== 'undefined' ? userEvents : []).slice()
      .filter(e=>e.adMs >= Date.now() - 3600000)
      .sort((a,b)=>a.adMs-b.adMs);
    if(evs.length){ nextEventTitle = evs[0].title; nextEventDate = new Date(evs[0].adMs).toDateString(); }
  }catch(e){}
  let tasksOpen = 0;
  try{ tasksOpen = (LSget('vikram_tasks', []) || []).filter(t=>!t.done).length; }catch(e){}
  return {
    type: 'widget',
...
function widgetFeedPush(){
  ...
  if(!local.enabled || !local.code) return;
  famEnsureAuth((ok)=>{
    if(!ok) return;
...
window.widgetFeedEnable = function(){
  ...
  famEnsureAuth((ok)=>{
    ...
    widgetFeedPush();
    if(!_widgetFeedTimer) _widgetFeedTimer = setInterval(widgetFeedPush, 600000); // re-push every 10 min
    H('success');
    T('📱 Widget feed is live!');
    if(typeof window.renderWidgetLiveSheet === 'function') window.renderWidgetLiveSheet();
  });
};
window.widgetFeedDisable = function(){
  const local = widgetFeedLocal();
  if(_widgetFeedTimer){ clearInterval(_widgetFeedTimer); _widgetFeedTimer = null; }
  const code = local.code;
...
window.widgetFeedCopyScriptable = function(){
  const local = widgetFeedLocal();
  if(!local.code){ T('⚠️ Turn the widget feed on first'); return; }
  ...
  }catch(e){ T('⚠️ Clipboard not available on this browser'); }
};
...
window.renderWidgetLiveSheet = function(){
  ...
  if(!local.enabled || !local.code){
    if(sub) sub.textContent = 'Auto-updating widget via Shortcuts/Tasker';
    body.innerHTML = `
  ...
  if(sub) sub.textContent = 'Feed is live · Code: ' + local.code;
  body.innerHTML = `
...
document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) widgetFeedPush(); });
...
(function setupBackGestureToHome(){
  ...
  function isAwayNow(){
    try{
      if(typeof _currentTab !== 'undefined' && _currentTab && _currentTab !== 'home') return true;
      const ov = document.getElementById('ov');
      if(ov && ov.classList.contains('open')) return true;
      const wl = document.getElementById('wlOverlay');
      if(wl && wl.style.display !== 'none' && wl.classList.contains('open')) return true;
      const fb = document.getElementById('fbOverlay');
      if(fb && fb.style.display !== 'none') return true;
    }catch(e){}
    return false;
  }
  ...
  function goHomeFromGesture(){
    ...
    const wl = document.getElementById('wlOverlay');
    if(wl){ wl.classList.remove('open'); wl.style.display = 'none'; }
    const fb = document.getElementById('fbOverlay');
  ...
  try{ history.replaceState({ vikramNav:'home' }, '', location.href); }catch(e){}
...
(function homeInsightsCards(){
  ...
  function buildGoldFxCardHTML(data, staleTs?){
    ...
    let html = '';
    if(hasMetal){
      if(data.gold) html += `<div class="ins-fx-row"><div class="ins-fx-name">🟡 Gold (Hallmark, per tola)</div><div class="ins-fx-val">Rs ${esc(data.gold)}</div></div>`;
      if(data.silver) html += `<div class="ins-fx-row"><div class="ins-fx-name">⚪ Silver (per tola)</div><div class="ins-fx-val">Rs ${esc(data.silver)}</div></div>`;
    }
    if(hasFx){
      data.fx.forEach(f=>{
        html += `<div class="ins-fx-row"><div class="ins-fx-name">${esc(f.code)}</div><div class="ins-fx-val">Rs ${esc(f.sell)}</div></div>`;
  ...
  window.renderHomeGoldFx = function(forceRefresh){
    ...
    if(fresh){ el.innerHTML = buildGoldFxCardHTML(cache.data); return; }
    ...
    Promise.resolve(typeof insightsFetchGoldFx==='function' ? insightsFetchGoldFx(cache) : null).then(()=>{
      ...
      if(updated && updated.data){ el.innerHTML = buildGoldFxCardHTML(updated.data); }
      else if(cache && cache.data){ el.innerHTML = buildGoldFxCardHTML(cache.data, cache.ts); }
      else { el.innerHTML = buildGoldFxCardHTML(null); }
