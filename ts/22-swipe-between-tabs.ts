...
(function(){
  ...
  function anyOverlayOpen(){
    ...
    if(fb && getComputedStyle(fb).display !== 'none') return true;
    const wl = document.getElementById('wlOverlay');
    if(wl && getComputedStyle(wl).display !== 'none') return true;
    const onb = document.getElementById('onbOverlay');
    if(onb && onb.classList.contains('show')) return true;
    return false;
  }
  ...
  function startsInHorizontalScroller(el){
    ...
    while(node && node !== document.body){
      if(node.classList && node.classList.contains('task-swipe-wrap')) return true;
  ...
  function onMove(e){
    ...
    if(horizontal) e.preventDefault(); // stop page from scrolling sideways while we handle it
  }
  ...
  function onEnd(e){
    ...
    if(dt > MAX_SWIPE_MS || Math.abs(dx) < MIN_SWIPE_PX || Math.abs(dx) < Math.abs(dy)) return;
