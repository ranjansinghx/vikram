...
function hamroLoadUpcomingFeed(){
  if(Array.isArray(hamroUpcomingCache)) return Promise.resolve(hamroUpcomingCache);
  if(hamroUpcomingLoadPromise) return hamroUpcomingLoadPromise;

  hamroUpcomingLoadPromise = (async () => {
    try{
      const res = await fetch(HAMRO_UPCOMING_URL, { cache: 'no-store' });
      if(!res.ok) throw new Error(`hamropatro_http_${res.status}`);
...
let cfg: any={greg:true,hol:true,holn:true,fd:1,tithi:false,weekNum:false,compact:false,hlwknd:true,
...
(function loadCfg(){
  try{
    ...
    if(stored&&typeof stored==='object'){
      if(typeof stored.greg==='boolean')          cfg.greg=stored.greg;
      if(typeof stored.hol==='boolean')           cfg.hol=stored.hol;
      if(typeof stored.holn==='boolean')          cfg.holn=stored.holn;
      if(typeof stored.fd==='number')             cfg.fd=stored.fd;
      if(typeof stored.tithi==='boolean')         cfg.tithi=stored.tithi;
      if(typeof stored.weekNum==='boolean')       cfg.weekNum=stored.weekNum;
      if(typeof stored.compact==='boolean')       cfg.compact=stored.compact;
      if(typeof stored.hlwknd==='boolean')        cfg.hlwknd=stored.hlwknd;
      if(typeof stored.focusMode==='boolean')     cfg.focusMode=stored.focusMode;
      if(typeof stored.simpleMode==='boolean')    cfg.simpleMode=stored.simpleMode;
      if(typeof stored.accentColor==='string')    cfg.accentColor=stored.accentColor;
      if(typeof stored.defReminder==='number')    cfg.defReminder=stored.defReminder;
      if(typeof stored.showCountBadge==='boolean')cfg.showCountBadge=stored.showCountBadge;
      if(typeof stored.showPinnedNote==='boolean')cfg.showPinnedNote=stored.showPinnedNote;
      if(typeof stored.cycleTopView==='string')   cfg.cycleTopView=stored.cycleTopView;
    }
  }catch(e){}
})();
...
function getKathmanduToday(){
  try{
    ...
    return new Date(Date.UTC(
      ...
      +p.find(x=>x.type==='month').value-1,
      +p.find(x=>x.type==='day').value,
      12,0,0  // noon UTC – avoids local-time date rollover in any timezone
    ));
  }catch(e){
...
function checkDateRollover(){
  try{
    ...
    if(fresh.getTime() === TODAY.getTime()) return;
    ...
    if(wasOnToday) selectedDay = {y:TODAYBS.y,m:TODAYBS.m,d:TODAYBS.d};
    if(wasViewingTodayMonth){ vY=TODAYBS.y; vM=TODAYBS.m; }
    if(typeof render==='function') render();
    if(typeof renderUpcoming==='function') renderUpcoming();
    if(typeof renderSelectedDay==='function') renderSelectedDay();
    if(typeof bdtRender==='function') bdtRender();
    if(typeof window.dcRender==='function') window.dcRender();
  }catch(e){}
}
...
function bsStart(bY,bM){
  ...
  if(bY>y||(bY===y&&bM>m)){
    while(!(y===bY&&m===bM)){days+=BS[y][m-1];m++;if(m>12){m=1;y++;}}
    return new Date(REF.getTime()+days*86400000);
  } else if(bY<y||(bY===y&&bM<m)){
    while(!(y===bY&&m===bM)){m--;if(m<1){m=12;y--;}days+=BS[y][m-1];}
    return new Date(REF.getTime()-days*86400000);
  }
...
function adToBs(ad){
  ...
  if(diff>=0){
    let r=diff;
    while(BS[y]&&r>=BS[y][m-1]){r-=BS[y][m-1];m++;if(m>12){m=1;y++;}}
    return{y,m,d:r+1};
  } else {
    let r=-diff-1;m--;if(m<1){m=12;y--;}
    while(BS[y]&&r>=BS[y][m-1]){r-=BS[y][m-1];m--;if(m<1){m=12;y--;}}
...
function getTithi(ad){
  ...
  let yr=y,mn=mo;if(mn<=2){yr--;mn+=12;}
...
function render(){
  ...
  const ord=[];for(let i=0;i<7;i++)ord.push((cfg.fd+i)%7);
  ...
  function getTithiLabel(ad){
    try{
      ...
      if(ti >= 0 && ti < 30) return PAKSHA_SHORT[ti] + TITHI_NUM[ti];
    }catch(e){}
    return '';
  }
  ...
  let tot=sDow+dims;if(tot%7!==0)tot+=7-tot%7;
  ...
  for(let i=0;i<tot;i++){
    ...
    if(i<sDow){
      const nd=pDims-sDow+i+1,ad=new Date(sAD);ad.setDate(ad.getDate()-(sDow-i));
      ...
      const nd=i-sDow-dims+1,ad=new Date(sAD);ad.setDate(ad.getDate()+(i-sDow));
      const wnHtml = (cfg.weekNum && isRowStart) ? `<div class="cell-weeknum">W${getWeekNum(ad)}</div>` : '';
      const tiHtml = cfg.tithi ? `<div class="cell-tithi">${getTithiLabel(ad)}</div>` : '';
      ...
      const nd=i-sDow+1,ad=new Date(sAD);ad.setDate(ad.getDate()+nd-1);
      ...
      let cls='cell active';
      if(isT)cls+=' today';else if(isH)cls+=' hol';else if(wk)cls+=' wk';
      ...
      if(cycleData.lastPeriodStart){
        ...
        const baseD=new Date(cycleData.lastPeriodStart);baseD.setHours(0,0,0,0);
        const dayN=Math.floor((+adMid-+baseD)/86400000);
        const dayInCyc=((dayN%cl2)+cl2)%cl2+1;
        if(dayInCyc===1)             chipArr.push(`<div class="cell-chip cell-chip-cyc">${cfg.compact?'':'🩸 Period'}</div>`);
        else if(dayInCyc===pl2)      chipArr.push(`<div class="cell-chip cell-chip-cyc">${cfg.compact?'':'Period end'}</div>`);
        else if(dayInCyc===fertD2)   chipArr.push(`<div class="cell-chip cell-chip-cyc">${cfg.compact?'':'🌸 Fertile'}</div>`);
        else if(dayInCyc===ovD2)     chipArr.push(`<div class="cell-chip cell-chip-cyc">${cfg.compact?'':'✨ Ovulation'}</div>`);
      }
      ...
      if(extraChips>0) chipsHtml+=`<div class="cell-chip cell-chip-more">+${extraChips}</div>`;
...
function prevMonth(){haptic('light');vM--;if(vM<1){vM=12;vY--;}render();}
function nextMonth(){haptic('light');vM++;if(vM>12){vM=1;vY++;}render();}
function goToday(){
  haptic('medium');
  vY=TODAYBS.y;vM=TODAYBS.m;selectedDay={y:TODAYBS.y,m:TODAYBS.m,d:TODAYBS.d};
...
function toggle3MView(){
  ...
  if(mm3Active){
    ...
    if(mmView){ mmView.style.display = 'block'; render3M(); }
    // Collapse upcoming to save space
    if(upcoming) upcoming.style.marginTop = '0';
  } else {
    btn.classList.remove('on');
    if(ccard) ccard.style.display = '';
    if(mmView) mmView.style.display = 'none';
    if(upcoming) upcoming.style.marginTop = '';
    render();
  }
}
...
function toggleScheduleView(){
  ...
  if(typeof weekViewActive!=='undefined' && weekViewActive){
    ...
    if(wvBtn) wvBtn.classList.remove('on');
    if(weekView) weekView.style.display='none';
    if(typeof wvStopNowLineTimer==='function') wvStopNowLineTimer();
  ...
  if(typeof dayViewActive!=='undefined' && dayViewActive){
    ...
    if(dvBtn) dvBtn.classList.remove('on');
    if(dayView) dayView.style.display='none';
    if(typeof dvStopNowLineTimer==='function') dvStopNowLineTimer();
    scheduleActive = false; // force toggle on
  }
  ...
  if(typeof monthTabActive!=='undefined' && monthTabActive){
    ...
    if(mtBtn) mtBtn.classList.remove('on');
    if(monthTabView) monthTabView.style.display='none';
    scheduleActive = false; // force toggle on
  }
  ...
  if(scheduleActive){
    ...
    if(ccard) ccard.style.display='none';
    if(schedView){ schedView.style.display=''; renderScheduleView(); }
...
function renderScheduleView(){
  ...
  function getDay(adMid){
    ...
    if(!dayMap[k]) dayMap[k] = {adMid, items:[]};
    return dayMap[k];
  }
  ...
  userEvents.forEach(ev => {
    ...
    function pushDay(occStart, occEnd){
      for(let s=new Date(occStart); s<=occEnd && s<=adEndMid; s=new Date(s.getTime()+msDay)){
        if(s < adStartMid) continue;
        const bs = adToBs(s);
        getDay(new Date(s)).items.push({type:'evt', ev, bs});
      }
    }
    if(!ev.recur && !ev.repeatYearly){ pushDay(startMid, endMid); return; }
    if(ev.repeatYearly && !ev.recur){
      for(let bsY=firstBsY; bsY<=lastBsY+1; bsY++){
        ...
        if(vS > adEndMid) break;
        pushDay(vS, new Date(vS.getTime()+spanDays*msDay));
      }
      return;
    }
    const r = ev.recur;
    if(!r||!r.freq){ pushDay(startMid,endMid); return; }
    const interval = r.interval || 1;
    let occ = 0, cur = new Date(startMid);
    const maxIter = 600; let iter = 0;
    while(cur <= adEndMid && iter++ < maxIter){
      if(r.endType==='count' && occ >= (r.endCount||1)) break;
      if(r.endType==='date' && r.endAdMs){
        const ed=new Date(r.endAdMs); const edM=new Date(ed.getFullYear(),ed.getMonth(),ed.getDate());
        if(cur>edM) break;
      }
      pushDay(cur, new Date(cur.getTime()+spanDays*msDay));
      occ++;
      if(r.freq==='daily') cur=new Date(cur.getTime()+interval*msDay);
      else if(r.freq==='weekly'){
        if(r.weekdays&&r.weekdays.length){
          cur=new Date(cur.getTime()+msDay);
          while(!r.weekdays.includes(cur.getDay())) cur=new Date(cur.getTime()+msDay);
        } else cur=new Date(cur.getTime()+interval*7*msDay);
      } else if(r.freq==='monthly'){
        const nm=new Date(cur); nm.setMonth(nm.getMonth()+interval); cur=nm;
      } else if(r.freq==='yearly'){
        const ny=new Date(cur); ny.setFullYear(ny.getFullYear()+interval); cur=ny;
      } else break;
    }
  });
  ...
  if(cycleData && cycleData.lastPeriodStart){
    ...
    while(new Date(cycleStart.getTime()+cl*msDay)<adStartMid) cycleStart=new Date(cycleStart.getTime()+cl*msDay);
    const phases=[
    ...
    try {
      ...
      _cycItems.forEach(ci => {
        ...
        if(phDay < adStartMid || phDay > adEndMid) return;
  ...
  const filterHtml = filterDefs.map(f=>
    `<button class="sv-filter-chip${curFilter===f.type?' active':''}" data-type="${f.type}" onclick="schedApplyFilter('${f.type}')">${f.icon} ${f.label}</button>`
  ).join('');
  ...
  for(let y=Math.max(firstBsY,2083); y<=Math.min(lastBsY,2086); y++){
    if(!BS[y]) continue;
    for(let m=1; m<=12; m++){
      const dims=BS[y][m-1]; if(!dims) continue;
      ...
      for(let d=1;d<=dims;d++){
        ...
        if(eventDayKeys.has(k)) evtDays.push({d,adMid2,k});
      }
  ...
  months.forEach(({y,m,dims,isCurrentMonth,isPastMonth,evtDays})=>{
    ...
    if(isCurrentMonth) todayMonthAnchorId=anchorId;
    ...
    function accentFor(it){
      if(it.type==='hol') return '#f5940a';
      if(it.type==='obs') return '#a855f7';
      if(it.type==='cyc') return '#ec4899';
      const ev = it.ev;
      if(ev && ev.color==='birthday') return '#ec4899';
    ...
    if(allDaysMeta.length===0) return;
    ...
    while(i < allDaysMeta.length){
      let j = i;
      while(j < allDaysMeta.length && allDaysMeta[j].adMid2.getDay() !== 6) j++;
      if(j < allDaysMeta.length) j++; else j = allDaysMeta.length;
      ...
      if(!hasContent){
        ...
        weekChunk.forEach(dayMeta=>{
          if(dayMeta.items.length===0 && !dayMeta.isToday) return;
  ...
  if(!bodyHtml){
    ...
    if(fabEmpty) fabEmpty.style.display = 'none';
    const addFabEmpty = document.getElementById('schedAddFab');
    if(addFabEmpty) addFabEmpty.style.display = 'flex';
    return;
  }
  ...
  requestAnimationFrame(()=>{
    try{
      if(window._schedMonthObserver) window._schedMonthObserver.disconnect();
      if(window._schedSpyTimer) clearTimeout(window._schedSpyTimer);
...
function toggleWeekView(){
  ...
  if(weekViewActive){
    ...
    if(scheduleActive){
      ...
      if(tabSchedule) tabSchedule.classList.remove('on');
      if(schedView) schedView.style.display='none';
      ...
      if(_saf) _saf.style.display = 'none';
    }
    // Deactivate day view if active
    if(typeof dayViewActive!=='undefined'&&dayViewActive){
      dayViewActive = false;
      if(tabDay) tabDay.classList.remove('on');
      if(dayView) dayView.style.display='none';
      if(typeof dvStopNowLineTimer==='function') dvStopNowLineTimer();
    }
    ...
    if(typeof monthTabActive!=='undefined'&&monthTabActive){
      ...
      const monthTabView = document.getElementById('monthTabView');
      if(tabMonth) tabMonth.classList.remove('on');
      if(monthTabView) monthTabView.style.display='none';
    }
    btn.classList.add('on');
    tabAll.classList.remove('on');
...
function toggleMonthTab(){
  ...
  if(monthTabActive){
    ...
    if(typeof dayViewActive!=='undefined'&&dayViewActive){
      dayViewActive = false;
      if(tabDayBtn) tabDayBtn.classList.remove('on');
      if(dayView) dayView.style.display='none';
      if(typeof dvStopNowLineTimer==='function') dvStopNowLineTimer();
    }
    ...
    if(weekViewActive){
      weekViewActive = false;
      if(tabWeekBtn) tabWeekBtn.classList.remove('on');
      if(weekView) weekView.style.display='none';
      if(typeof wvStopNowLineTimer==='function') wvStopNowLineTimer();
    }
    btn.classList.add('on');
...
function renderMonthTabView(){
  ...
  // Build event map
  const dayMap = {};
  for(let d=1;d<=dims;d++) dayMap[d]={hols:[],evts:[],cycs:[],obs:[]};

  for(const key of Object.keys(HOL)){
    const[ky,km,kd]=key.split('-').map(Number);
    if(ky===y&&km===m&&dayMap[kd]) dayMap[kd].hols.push(HOL[key]);
  }
  for(const key of Object.keys(OBS)){
    const[ky,km,kd]=key.split('-').map(Number);
    if(ky===y&&km===m&&dayMap[kd]) dayMap[kd].obs.push(OBS[key]);
  }
  ...
  userEvents.forEach(ev=>{
    ...
    if(!ev.recur && !ev.repeatYearly){
      ...
      const r = ev.recur;
      if(!r||!r.freq){ occurrences.push({oStart:startMid, oEnd:endMid}); }
      else {
        const interval=r.interval||1;
        let occ=0, cur=new Date(startMid), maxIter=600, iter=0;
        while(cur<=mEndMid && iter++<maxIter){
          if(r.endType==='count'&&occ>=(r.endCount||1)) break;
          if(r.endType==='date'&&r.endAdMs){const ed=new Date(r.endAdMs);const edM=new Date(ed.getFullYear(),ed.getMonth(),ed.getDate());if(cur>edM) break;}
          occurrences.push({oStart:new Date(cur), oEnd:new Date(cur.getTime()+spanDays*msDay2)});
          occ++;
          if(r.freq==='daily') cur=new Date(cur.getTime()+interval*msDay2);
          else if(r.freq==='weekly'){
            if(r.weekdays&&r.weekdays.length){cur=new Date(cur.getTime()+msDay2);while(!r.weekdays.includes(cur.getDay()))cur=new Date(cur.getTime()+msDay2);}
            else cur=new Date(cur.getTime()+interval*7*msDay2);
          } else if(r.freq==='monthly'){const nm=new Date(cur);nm.setMonth(nm.getMonth()+interval);cur=nm;}
          else if(r.freq==='yearly'){const ny=new Date(cur);ny.setFullYear(ny.getFullYear()+interval);cur=ny;}
          else break;
        }
      }
    }
    occurrences.forEach(({oStart, oEnd})=>{
      if(oEnd<mStartMid||oStart>mEndMid) return;
      for(let d=1;d<=dims;d++){
        const adD=bsToAd(y,m,d);
        const adMid=new Date(adD.getFullYear(),adD.getMonth(),adD.getDate());
        if(adMid>=oStart&&adMid<=oEnd && !dayMap[d].evts.includes(ev)) dayMap[d].evts.push(ev);
      }
    });
  });
  ...
  cycItems.forEach(ci=>{
    if(ci.ky===y&&ci.km===m&&dayMap[ci.kd]) dayMap[ci.kd].cycs.push(ci);
  });
  ...
  for(let i=0;i<startDow;i++) html+=`<div class="mtv-cell mtv-cell-empty"></div>`;
  ...
  for(let d=1;d<=dims;d++){
    ...
    let chipsHtml = visChips.join('');
    if(extraChips>0) chipsHtml += `<div class="mtv-chip mtv-chip-more">+${extraChips}</div>`;
...
function renderWeekView(){
  ...
  days.forEach(day=>{
    ...
    if(HOL[k]) day.hols.push(HOL[k]);
    if(OBS[k]) day.obs.push(OBS[k]);
  });
  ...
  userEvents.forEach(ev=>{
    ...
    function tryPush(occStart, occEnd){
      days.forEach(day=>{
        if(day.adMid>=occStart && day.adMid<=occEnd) day.events.push(ev);
      });
    }

    if(!ev.recur&&!ev.repeatYearly){ tryPush(startMid,endMid); return; }
    if(ev.repeatYearly&&!ev.recur){
    ...
    const r=ev.recur;
    if(!r||!r.freq){tryPush(startMid,endMid);return;}
    const interval=r.interval||1;
    let occ=0,cur=new Date(startMid);
    const maxIter=600;let iter=0;
    while(cur<=weekEnd&&iter++<maxIter){
      ...
      if(r.freq==='daily') cur=new Date(cur.getTime()+interval*msDay);
        ...
        if(r.weekdays&&r.weekdays.length){cur=new Date(cur.getTime()+msDay);while(!r.weekdays.includes(cur.getDay()))cur=new Date(cur.getTime()+msDay);}
        else cur=new Date(cur.getTime()+interval*7*msDay);
      } else if(r.freq==='monthly'){const nm=new Date(cur);nm.setMonth(nm.getMonth()+interval);cur=nm;}
      else if(r.freq==='yearly'){const ny=new Date(cur);ny.setFullYear(ny.getFullYear()+interval);cur=ny;}
      else break;
  ...
  requestAnimationFrame(()=>{
    ...
    if(scroll) scroll.scrollTop=nowTopPx-120;
  });
  wvStartNowLineTimer();
}
...
function wvStartNowLineTimer(){
  if(_wvNowLineIv) clearInterval(_wvNowLineIv);
  _wvNowLineIv=setInterval(wvUpdateNowLine,30000);
}
function wvStopNowLineTimer(){
  if(_wvNowLineIv){ clearInterval(_wvNowLineIv); _wvNowLineIv=null; }
}
function wvUpdateNowLine(){
  if(!weekViewActive||weekViewOffset!==0) return;
  const CELL_H=54;
  const nstNow=new Date(Date.now()+345*60000);
  const nowMins=nstNow.getUTCHours()*60+nstNow.getUTCMinutes();
...
function renderDayView(){
  ...
  if(HOL[bsKey]) hols.push(HOL[bsKey]);
  if(OBS[bsKey]) obsList.push(OBS[bsKey]);
  ...
  userEvents.forEach(ev=>{
    ...
    function check(occStart, occEnd){
      if(targetMid>=occStart && targetMid<=occEnd) dayEvents.push(ev);
    }
    if(!ev.recur&&!ev.repeatYearly){ check(startMid,endMid); return; }
    if(ev.repeatYearly&&!ev.recur){
    ...
    const r=ev.recur;
    if(!r||!r.freq){check(startMid,endMid);return;}
    const interval=r.interval||1;
    let occ=0,cur=new Date(startMid);
    const maxIter=600;let iter=0;
  ...
  requestAnimationFrame(()=>{
    ...
    if(scroll) scroll.scrollTop = isToday ? Math.max(0,nowTopPx-120) : 8*CELL_H-60;
  });
...
function dvStartNowLineTimer(){
  if(_dvNowLineIv) clearInterval(_dvNowLineIv);
  _dvNowLineIv = setInterval(dvUpdateNowLine, 30000);
}
function dvStopNowLineTimer(){
  if(_dvNowLineIv){ clearInterval(_dvNowLineIv); _dvNowLineIv=null; }
}
function dvUpdateNowLine(){
  if(!dayViewActive||dayViewOffset!==0) return;
  const CELL_H=54;
  const nstNow=new Date(Date.now()+345*60000);
  const nowMins=nstNow.getUTCHours()*60+nstNow.getUTCMinutes();
  const nowTopPx=(nowMins/60)*CELL_H;
  const el=document.getElementById('dvNowLine');
  if(el) el.style.top=nowTopPx+'px';
}


function wvCellTap(e, dayIdx, hour, min){
  // Don't trigger if an event/banner was clicked
  if(e.target.closest('.wv-event,.wv-hol-banner,.wv-obs-banner,.wv-nav-btn')) return;
  const msDay = 86400000;
...
function render3M(){
  ...
  months.forEach(({y, m}, idx) => {
    ...
    let tot = sDow + dims;
    if(tot % 7 !== 0) tot += 7 - tot % 7;

    // Day header row
    const dn = lang==='ne' ? DNE : DEN;
    const ord = [];
    for(let i=0;i<7;i++) ord.push((cfg.fd+i)%7);
    ...
    for(let i=0;i<tot;i++){
      ...
      if(i < sDow){
        ...
        const nd = i - sDow + 1;
        const adD = new Date(sAD); adD.setDate(adD.getDate() + nd - 1);
        const adMid = new Date(adD.getFullYear(), adD.getMonth(), adD.getDate());
        ...
        const dots = [];
        if(h) dots.push('<div class="mm-dot hol"></div>');
        else if(getObs(y, m, nd)) dots.push('<div class="mm-dot evt"></div>');

        userEvents.forEach(ev => {
          if(eventMatchesDate(ev, y, m, nd)) dots.push('<div class="mm-dot evt"></div>');
        });
        ...
        if(cycleData.lastPeriodStart){
          ...
          const dayN = Math.floor((+adMid-+baseD)/86400000);
          const dayInCyc = ((dayN%cl2)+cl2)%cl2+1;
          if(dayInCyc>=1 && dayInCyc<=pl2) dots.push('<div class="mm-dot cyc"></div>');
          else if(dayInCyc>=fertD2 && dayInCyc<=ovD2) dots.push('<div class="mm-dot cyc"></div>');
        }
    ...
    html += `</div></div>`;
    if(idx < 2) html += '<div class="mm-sep"></div>';
  });
...
function mmPrev(){
  haptic('light');
  vM -= 3; while(vM < 1){ vM += 12; vY--; }
  render(); render3M();
}

function mmNext(){
  haptic('light');
  vM += 3; while(vM > 12){ vM -= 12; vY++; }
  render(); render3M();
}
...
function getCurrentWeekRow(){
  ...
  const dims = BS[vY]?.[vM-1] ?? 30;
  let focusD = null;
  if(vY === TODAYBS.y && vM === TODAYBS.m) focusD = TODAYBS.d;
  else if(selectedDay.y === vY && selectedDay.m === vM) focusD = selectedDay.d;
  if(focusD === null) focusD = 1;
...
function getCalHeights(){
  ...
  for(let r = 0; r < currentRow; r++){
    ...
    if(cell) collapsedOffset += cell.offsetHeight + rowGap;
  }
...
function renderWeekDots(total, active){
  ...
  wrap.innerHTML = Array.from({length:total},(_,i)=>
    `<div class="cal-week-dot${i===active?' active':''}"></div>`
  ).join('');
}
...
function applyCalHeight(animate, velocity?){
  ...
  if(firstCell && firstCell.offsetHeight === 0) return;
  ...
  if(lbl) lbl.textContent = calExpanded ? 'Show week' : 'Show month';
...
function toggleCalExpand(){
  ...
  if(handle) spawnRipple(handle.getBoundingClientRect());
}
...
(function(){
  ...
  function updateLiveRowDim(frac){
    ...
    cells.forEach((c, i)=>{
      const row = Math.floor(i / 7);
      if(row === currentRow){ c.style.opacity=''; c.style.filter=''; return; }
      const dist = Math.abs(row - currentRow);
  ...
  function clearLiveRowDim(){
    ...
    grid.querySelectorAll('.cell').forEach(c=>{
      c.style.opacity=''; c.style.filter=''; c.style.transform='';
    });
  }

  function onTouchStart(e){
    if(e.target.closest('.nbtn')) return;
    if(document.body.classList.contains('simple-calendar-mode')) return; // always full month in Simple mode
    if(e.touches.length > 1) return;
    const t = e.touches[0];
    startY = t.clientY; startX = t.clientX;
    lastY = startY; lastT = Date.now(); velocity = 0;
    isDragging = false; isVertical = false;
    ...
    const clip = getClip(); const grid = getGrid();
  ...
  function onTouchMove(e){
    ...
    if(!isDragging){
      ...
      if(Math.abs(dx) > Math.abs(dy)){ isVertical=false; isDragging=true; return; }
      isVertical = true; isDragging = true;
    }
    ...
    if(frac < 0) frac = frac / (1 - frac * 3);
    if(frac > 1) frac = 1 + (frac-1) / (1 + (frac-1) * 3);
    ...
    const clip = getClip(); const grid = getGrid(); const card = getCard();
    if(clip) clip.style.height          = liveH + 'px';
    if(grid) grid.style.transform       = `translateY(${liveOffset}px)`;
    ...
    if(grid){
      ...
      grid.querySelectorAll('.cell').forEach((c, i)=>{
        const row = Math.floor(i / 7);
        if(row === currentRow){ c.style.visibility = ''; return; }
    ...
    const lbl = document.getElementById('calHandleLabel');
    if(lbl) lbl.textContent = frac > 0.5 ? 'Show week' : 'Show month';
  ...
  function onTouchEnd(e){
    if(!isDragging || !isVertical){
      isDragging = false; isVertical = false; return;
    }
    isDragging = false; isVertical = false;
    ...
    if(!calExpanded){
      if(frac > COMMIT_FRAC || isFlickDown) calExpanded = true;
    } else {
      if(frac < 1-COMMIT_FRAC || isFlickUp) calExpanded = false;
    }
    if(calExpanded !== prevExpanded) haptic('light');
    ...
    if(!calExpanded && dy > 0) spawnRipple(getCard()?.getBoundingClientRect() ?? {left:0,top:0,width:300,height:200});
...
function openDay(bY,bM,bD){
  ...
  if(panchEl && typeof getPanchangText === 'function') panchEl.textContent = getPanchangText(ad);
...
function closeCycleSheet(){
  ...
  if(window.cmState && window.cmState.running) window.cmStopGame && window.cmStopGame();
  if(window.pomoState && window.pomoState.running){ window.pomoToggle && window.pomoToggle(); }
  if(window.tasksFlushSave) window.tasksFlushSave();
...
function toggleDark(){
  ...
  if(autoTheme){
    ...
    if(tgA) tgA.classList.remove('on');
...
function applyTheme(){
  ...
  if(meta) meta.setAttribute('content',dark?'#0d0f16':'#ECEEF8');
  // Settings toggle
  ...
  if(sub) sub.textContent=dark?'Currently using dark theme':'Currently using light theme';
  if(icon){ icon.textContent=dark?'🌙':'☀️'; icon.style.background=dark?'rgba(80,90,180,.15)':'rgba(240,180,40,.15)'; }
...
let autoTheme=(function(){
  try{ return localStorage.getItem('vikram_theme_auto')==='1'; }catch(e){ return false; }
})();
...
function toggleAutoTheme(){
  ...
  if(tg) tg.classList.toggle('on',autoTheme);
  if(autoTheme){
    applyAutoThemeNow();
    if(!_autoThemeTimer) _autoThemeTimer=setInterval(applyAutoThemeNow,300000); // re-check every 5 min
  } else {
    if(_autoThemeTimer){ clearInterval(_autoThemeTimer); _autoThemeTimer=null; }
...
function setLang(l){
  ...
  if(cEN) cEN.classList.toggle('on',l==='en');
  if(cNE) cNE.classList.toggle('on',l==='ne');
  const sub=document.getElementById('sLangSub');
  if(sub) sub.textContent=l==='ne'?'Currently Nepali (नेपाली)':'Currently English';
  ...
  document.getElementById('ov')?.classList.remove('open');
  render();renderUpcoming();renderSelectedDay();
}
...
function doConvBS(){
  ...
  if(!y||!m||!d){alert('Please fill all fields.');return;}
  if(!BS[y]){alert('Year out of supported range (2080–2086).');return;}
  const maxD=BS[y][m-1];
  if(d<1||d>maxD){alert(`Day must be between 1 and ${maxD} for this month.`);return;}
  const ad=bsToAd(y,m,d);
...
function safeCss(val, fallback){
  ...
  if(/^(#[0-9a-fA-F]{3,8}|rgba?\([^)]{0,60}\)|[a-zA-Z]{1,30})$/.test(val.trim())) return val.trim();
  return fallback||'';
}

let userEvents=(function(){try{return JSON.parse(localStorage.getItem('vikram_events')||'[]');}catch(e){return [];}})();
let upTab='hol';
...
function saveEvents(){
  try{
    ...
    if(e && (e.name==='QuotaExceededError'||e.code===22||e.code===1014)){
      ...
      _lsPrune();
      try{ localStorage.setItem('vikram_events',JSON.stringify(userEvents)); }catch(e2){}
    }
  }
}
...
function _lsPrune(){
  ...
  disposable.forEach(k=>{ try{ localStorage.removeItem(k); }catch(e){} });
...
function eventMatchesDate(ev, y, m, d){
  ...
  if(ev.repeatYearly && !ev.recur){
    if(y < ev.bsY) return false;
    if(spanDays <= 0) return m === ev.bsM && d === ev.bsD;
  ...
  const r = ev.recur;
  if(!r || !r.freq) return dayMid >= startMid && dayMid <= endMid;
  ...
  if(r.endType === 'date' && r.endAdMs){
    ...
    if(dayMid > endDMid) return false;
  }
  ...
  if(r.freq === 'daily'){
    ...
    if(r.endType === 'count' && occN >= (r.endCount || 1)) return false;
    ...
    if(r.endType === 'count'){
      ...
      for(let w = 0; w * interval * 7 * msD <= +dayMid - +startMid; w++){
        ...
        weekdays.forEach(wd=>{
          ...
          if(occD >= startMid && occD <= dayMid) count++;
        });
      }
      if(count > (r.endCount || 1)) return false;
    }
    return true;
  }
  ...
  if(r.freq === 'monthly'){
    ...
    if(dayMid.getDate() < effectiveDOM || dayMid.getDate() > effectiveDOM + spanDays) return false;
    // months elapsed
    const monthsElapsed = (dayMid.getFullYear() - startMid.getFullYear()) * 12
      + (dayMid.getMonth() - startMid.getMonth());
    if(monthsElapsed < 0 || monthsElapsed % interval !== 0) return false;
    if(r.endType === 'count'){
      const occN = Math.floor(monthsElapsed / interval);
      if(occN >= (r.endCount || 1)) return false;
    }
  ...
  if(r.freq === 'yearly'){
    ...
    if(yearsElapsed % interval !== 0) return false;
    if(r.endType === 'count'){
...
function formatEventRange(ev){
  ...
  if(startMid.getTime() === endMid.getTime()) return startStr;
...
function openSheet(t){
  ...
  closeAll();
  if(settingsWasActive) document.getElementById('settingsView')?.classList.add('active');
  ...
  if(!isFullscreen) document.getElementById('ov').classList.add('open');
  ...
  if(t==='tasks'){
    document.querySelector('.phone')?.classList.add('cycle-dim');
    if(window.tasksShowList) window.tasksShowList();
  }
  if(t==='kharcha'){
    document.querySelector('.phone')?.classList.add('cycle-dim');
  ...
  if(t==='add'){
    ...
    if(catPicker){
      ...
      if(defaultBtn && typeof selectEvCat==='function') selectEvCat(defaultBtn);
    }
...
function openAddForDayTime(bsY,bsM,bsD,startH,startMin){
  ...
  setTimeout(()=>{
    ...
    if(st) st.value=`${pad(startH)}:${pad(startMin)}`;
    if(et) et.value=`${pad(endH)}:${pad(startMin)}`;
    onEvTimeChange();
  },50);
}
...
function onEvTimeChange(){
  ...
  if(lbl) lbl.style.display=hasTime?'none':'';
  if(clearBtn) clearBtn.style.display=hasTime?'':'none';
...
function loadNotifHistory(){
  try{ return JSON.parse(localStorage.getItem(NOTIF_HIST_KEY)||'[]'); }catch(e){ return []; }
}
function saveNotifHistory(list){
  try{ localStorage.setItem(NOTIF_HIST_KEY, JSON.stringify(list)); }catch(e){}
}
function logNotifHistory(title,body,tag){
  ...
  if(list.length>NOTIF_HIST_MAX) list.length=NOTIF_HIST_MAX;
...
function relTimeShort(ts){
  ...
  if(min<1) return lang==='ne'?'अहिले':'Just now';
...
function updateNotifCard(){
  ...
  if(tgl){ tgl.classList.toggle('on',active); tgl.disabled=perm==='denied'; tgl.style.opacity=perm==='denied'?'.4':'1'; }
...
async function handleNotifToggle(){
  ...
  if(Notification.permission!=='granted'){
    ...
    if(p!=='granted'){updateNotifCard();return;}
    notifEnabled=true;
  } else {
...
async function checkEventReminders(){
  ...
  for(const ev of userEvents){
    ...
    if(!localStorage.getItem(key1day) && nowMs >= minus1day && nowMs < minus1day + WINDOW_MS){
      ...
      await showNotif(`📅 Tomorrow: ${ev.title}`,
        `Event tomorrow · ${dateStr}`,
        `1day-${ev.adMs}`);
    }
    ...
    if(!localStorage.getItem(key2hr) && nowMs >= minus2hr && nowMs < minus2hr + WINDOW_MS){
      ...
      await showNotif(`⏰ In 2 hours: ${ev.title}`,
        `Starting in 2 hours · ${dateStr}`,
        `2hr-${ev.adMs}`);
    }
    ...
    if(!localStorage.getItem(key30min) && nowMs >= minus30min && nowMs < minus30min + WINDOW_MS){
      ...
      await showNotif(`🔔 In 30 min: ${ev.title}`,
        `Starting in 30 minutes · ${dateStr}`,
        `30min-${ev.adMs}`);
    }
  }
}
...
function renderTodayCard(){
  ...
  try{
    ...
    try{
      if(typeof getCycleMode === 'function' && getCycleMode() === 'pregnant' && cycleData.pregnancy && cycleData.pregnancy.lmpDate){
        ...
        const ph = getPhase();
        if(ph) cycSub = `${ph.name} \u00b7 Day ${ph.dayIn} of ${ph.of}`;
      }
    }catch(e){}
  ...
  if(typeof renderFestivalBanner === 'function') renderFestivalBanner();
...
function getNextFestival(){
  try{
    ...
    for(const key in HOL){
      ...
      if(diff <= 0) continue;
      if(!best || diff < best.diff) best = { key, ky, km, kd, diff, hol: HOL[key] };
    }
    return best;
  }catch(e){ return null; }
}
function renderFestivalBanner(){
  ...
  el.onclick = function(){ if(typeof openDay==='function') openDay(nxt.ky,nxt.km,nxt.kd); };
...
window.todayGoToTasks = function(){
  ...
  if(typeof setUpView === 'function') setUpView('tasks');
};
...
async function checkTaskReminders(){
  ...
  let tasks;
  try{ tasks = JSON.parse(localStorage.getItem('vikram_tasks_v1') || '[]'); }catch(e){ tasks = []; }
  ...
  if(nowHM < REMIND_TIME || nowHM >= _addMinutesHM(REMIND_TIME, 2)) return;
...
async function checkCycleReminders(){
  ...
  if(!periodRemindersOn && !contraOn && !phaseRemindersOn) return;
  ...
  if(periodRemindersOn && pr.bbtMorning){
    ...
    const alreadyLogged = (cycleData.bbtLogs||[]).some(l=>{
      const sd = new Date(l.date); const sm = new Date(sd.getFullYear(),sd.getMonth(),sd.getDate()).getTime();
      return sm === todayMid;
    });
...
function _phaseReminderSubtitle(r){
  if(r.triggerType==='phase_start')        return `Start of your ${r.triggerPhase} phase`;
  if(r.triggerType==='phase_day')          return `Day ${r.triggerDayIn} of your ${r.triggerPhase} phase`;
  if(r.triggerType==='cycle_day')          return `Cycle day ${r.triggerCycleDay}`;
  if(r.triggerType==='days_before_period') return `${r.triggerDaysBefore} days before your next period`;
  return '';
}
...
function savePhaseReminder(){
  ...
  if(!label){ if(typeof vikramToast==='function') vikramToast('⚠️ Add a name for this reminder'); return; }
  ...
  if(idx>=0) cycleData.phaseReminders[idx] = entry;
  else cycleData.phaseReminders.push(entry);
  saveCycle();
  closePhaseReminderSheet();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Reminder saved');
}
function deletePhaseReminder(id){
  ...
  saveCycle();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('🗑 Reminder removed');
}
function _phaseReminderShouldFireToday(r){
  ...
  if(r.triggerType === 'cycle_day')          return day === (r.triggerCycleDay||1);
  if(r.triggerType === 'days_before_period') return (cl - day + 1) === (r.triggerDaysBefore||2);

  const ovDay   = cl - 14;
  const follEnd = ovDay - 3;
  const ovEnd   = ovDay + 2;
  let phase, dayInPhase;
  if(day<=pl)        { phase='Menstrual';  dayInPhase=day; }
  else if(day<=follEnd){ phase='Follicular'; dayInPhase=day-pl; }
  else if(day<=ovEnd)  { phase='Ovulation';  dayInPhase=day-(ovDay-2)+1; }
  else                  { phase='Luteal';     dayInPhase=day-ovEnd; }

  if(r.triggerType === 'phase_start') return phase===r.triggerPhase && dayInPhase===1;
  if(r.triggerType === 'phase_day')   return phase===r.triggerPhase && dayInPhase===(r.triggerDayIn||1);
  return false;
}
...
function recurLabel(ev){
  ...
  if(ev.repeatYearly) return 'yearly';
  return '';
}

function recurSummaryText(){
  const r = window._recurState;
  if(!r.enabled) return 'No repeat';
  const n = r.interval || 1;
  ...
  if(r.endType === 'count') s += ` · ${r.endCount||5} times`;
...
function updateRecurSummary(){
  ...
  if(el) el.textContent = recurSummaryText();
  if(sub) sub.textContent = window._recurState.enabled ? recurSummaryText() : 'Never';
}
...
function updateRecurIntervalUnit(){
  ...
  if(unit) unit.textContent = n === 1 ? base+'(s)' : base+'s';
}
...
function toggleRecurPanel(){
  ...
  if(window._recurState.enabled) setRecurFreq(window._recurState.freq || 'daily');
  updateRecurSummary();
}
...
function setRecurFreq(freq){
  ...
  if(wdSec) wdSec.style.display = freq === 'weekly' ? 'block' : 'none';
...
function stepRecurInterval(delta){
  ...
  if(el) el.textContent = state.interval;
  updateRecurIntervalUnit();
  updateRecurSummary();
}
...
function toggleRecurWd(wd){
  ...
  if(idx === -1) state.weekdays.push(wd);
  else if(state.weekdays.length > 1) state.weekdays.splice(idx,1); // keep at least 1
  document.querySelectorAll('.recur-wd-btn').forEach(b=>{
...
function setRecurEnd(type){
  ...
  if(countSec) countSec.style.display = type === 'count' ? 'flex' : 'none';
  if(dateSec)  dateSec.style.display  = type === 'date'  ? 'block': 'none';
  if(type === 'date') updateRecurEndDay();
  updateRecurSummary();
}
...
function stepRecurCount(delta){
  ...
  if(el) el.textContent = window._recurState.endCount;
  updateRecurSummary();
}
...
function resetRecurPanel(){
  ...
  if(ivEl) ivEl.textContent = '1';
  const ctEl = document.getElementById('recurEndCountVal');
  if(ctEl) ctEl.textContent = '5';
  ...
  const wdSec = document.getElementById('recurWeekdaySection');
  if(wdSec) wdSec.style.display = 'none';
}
...
async function saveEv(){
  ...
  if(!bsY||!bsM||!bsD||!endBsY||!endBsM||!endBsD){alert('Please select a valid date.');return;}
  ...
  const evData={
    ...
    color: ((document.querySelector('#evColorPicker .ae-seg-btn.on, #evColorPicker .ae-cat-card.on, #evColorPicker .ae-cat-pill.on, #evColorPicker .ev-cat-btn.on') || {}) as any).dataset?.color || 'personal',
  ...
  if(Notification.permission==='granted'){
    ...
    await showNotif(
      `✅ Event Saved — ${t}`,
      `${MEN[bsM-1]} ${bsD}, ${bsY} BS · ${adStr}${note?' · '+note:''}`,
      `saved-${Date.now()}`
    );
  } else {
...
function switchUpTab(t){ /* retired — tabs removed */ }
...
function buildCycleUpcomingItems(todayMid, windowEnd){
  ...
  if(cycleStart > windowEnd) return [];
  ...
  for(let start = new Date(cycleStart); start <= windowEnd; start = new Date(start.getTime() + cl * msDay)){
    ...
    const periodStartLogged = isCurrentCycle && periodStartLogs.some(l => {
      const ld = new Date(l.date); const lMid = new Date(ld.getFullYear(),ld.getMonth(),ld.getDate());
...
function relLabel(diff){
  ...
  if(diff<=6)  return{text:`In ${diff} days`,cls:'soon'};
  if(diff<=13) return{text:'Next week',cls:'soon'};
  const wk=Math.round(diff/7);
  if(wk<=4)    return{text:`In ${wk} week${wk>1?'s':''}`,cls:'soon'};
  return{text:`In ~1 month`,cls:'red'};
}

// Icon & tag config per event type
function upItemMeta(type, subtype?){
  if(type==='hol') return {icon:'🇳🇵', iconCls:'hol', tagLabel:'HOLIDAY',    tagCls:'hol'};
  if(type==='evt') return {icon:'📌',  iconCls:'evt', tagLabel:'EVENT',      tagCls:'evt'};
  // cycle subtypes
  const cm={
...
function setUpView(v){
  ...
  if(btnL) btnL.classList.toggle('on', v==='upcoming');
  if(btnM) btnM.classList.toggle('on', v==='month');
  if(btnT) btnT.classList.toggle('on', v==='tasks');
  const tasksView = document.getElementById('upTasksView');
  if(tasksView) tasksView.style.display = v==='tasks' ? '' : 'none';
  if(v==='tasks'){
    const upList = document.getElementById('upList');
    const upMonthView = document.getElementById('upMonthView');
    if(upList) upList.style.display = 'none';
    if(upMonthView) upMonthView.style.display = 'none';
    if(typeof window.tasksShowList === 'function') window.tasksShowList();
    ...
    try{
      ...
      if(countEl){ countEl.textContent = String(incomplete); countEl.title = incomplete + ' tasks left'; countEl.classList.add('tasks-mode'); }
    }catch(e){}
  } else {
    renderUpcoming();
  }
...
function renderMonthView(){
  ...
  // Empty leading cells
  for(let i=0;i<startDow;i++) cal+=`<div class="mv-cell dim"><span class="mv-dnum"></span></div>`;
  ...
  for(let d=1;d<=dims;d++){
    ...
    if(extra > 0) chips += `<div class="mv-chip mv-chip-more">+${extra}</div>`;
  ...
  if(mvSelectedDay&&mvSelectedDay.y===y&&mvSelectedDay.m===m){
    ...
    for(let d=1;d<=dims;d++){
      const dm = dayMap[d];
      if(!dm.hols.length&&!dm.evts.length&&!dm.cycs.length) continue;
      hasAny=true;
      const adD=bsToAd(y,m,d);
    ...
    if(!hasAny) listHtml=`<div class="mv-empty">No events this month</div>`;
  }
...
function renderUpcoming(){
  ...
  if(!hamroUpcomingLoadPromise && !Array.isArray(hamroUpcomingCache)) hamroLoadUpcomingFeed().then(()=>{ if(typeof renderUpcoming === 'function') renderUpcoming(); });
  ...
  if(hamroFeed){
    hamroFeed.forEach(src=>{
      ...
      if(!BS[src.y] || !BS[src.y][src.m - 1]) return;
  ...
  userEvents.forEach(ev=>{
    ...
    function pushOcc(occStartMid, occEndMid, rangeLabel?){
      if(occEndMid < todayMid || occStartMid > windowEnd) return;
    ...
    // Full recurrence engine
    const r = ev.recur;
    if(!r || !r.freq){ pushOcc(startMid, endMid); return; }
    ...
    function withinEndCond(occN, occStartMid){
      ...
      if(r.endType === 'date' && r.endAdMs){
        ...
        if(occStartMid > edMid) return false;
      }
      return true;
    }
    ...
    if(r.freq === 'daily'){
      for(let occN = 0; occN < maxOcc; occN++){
        ...
        if(oStart > windowEnd) break;
        if(!withinEndCond(occN, oStart)) break;
    ...
    if(r.freq === 'weekly'){
      ...
      for(let wk = 0; wk < maxOcc; wk++){
        ...
        weekdays.forEach(wd=>{
          ...
          if(oStart < startMid || oStart > windowEnd) return;
          if(!withinEndCond(occCount, oStart)) return;
          const oEnd = new Date(oStart.getTime() + spanDays * msDay);
          pushOcc(oStart, oEnd);
          occCount++;
        });
    ...
    if(r.freq === 'monthly'){
      for(let mo = 0; mo < maxOcc; mo++){
        ...
        if(oStart > windowEnd) break;
        if(!withinEndCond(mo, oStart)) break;
        const oEnd = new Date(oStart.getTime() + spanDays * msDay);
        pushOcc(oStart, oEnd);
      }
      return;
    ...
    if(r.freq === 'yearly'){
      for(let yr = 0; yr < maxOcc; yr++){
        ...
        if(!BS[targetBsY]) break;
        const vStart = bsToAd(targetBsY, ev.bsM, ev.bsD);
        const vSMid = new Date(vStart.getFullYear(), vStart.getMonth(), vStart.getDate());
        if(vSMid > windowEnd) break;
        if(!withinEndCond(yr, vSMid)) break;
        const vEMid = new Date(vSMid.getTime() + spanDays * msDay);
        pushOcc(vSMid, vEMid);
      }
      return;
  ...
  const list=document.getElementById('upList');
  const countEl=document.getElementById('upCount');
  if(countEl){ countEl.textContent=String(items.length); countEl.classList.remove('tasks-mode'); countEl.title=''; countEl.style.display=cfg.showCountBadge?'':'none'; }
...
function renderSchedule(){
  ...
  for(const key of Object.keys(HOL)){
    ...
    const ad = bsToAd(ky,km,kd);
    const adMid = new Date(ad.getFullYear(), ad.getMonth(), ad.getDate());
    if(adMid < todayMid || adMid > windowEnd) continue;
  ...
  const cycleAnchorMid = (()=>{
    try{
      ...
      if(!raw || isNaN(+raw)) return todayMid;
      return new Date(raw.getFullYear(), raw.getMonth(), raw.getDate());
    }catch(e){ return todayMid; }
  })();
  ...
  if(sub) sub.textContent = `${totalItems} event${totalItems!==1?'s':''} in next ${SCHED_DAYS} days`;
...
function renderSelectedDay(){
  ...
  if(title) title.textContent=lang==='ne'?'चयनित दिन':'Selected Day';
...
function upRemoveEv(el){
  ...
  if(isNaN(i) || i < 0 || i >= userEvents.length){
    ...
    if(i < 0) i = userEvents.findIndex(e => e.adMs === adMs);
  }
  if(i < 0 || i >= userEvents.length) return;
  removeEv(i);
}
...
function mpPrevYear(){
  ...
  if(mpYear>minY){ mpYear--; renderMpGrid(); }
}
function mpNextYear(){
  haptic('light');
  const maxY=Object.keys(BS).map(Number).sort((a,b)=>b-a)[0];
  if(mpYear<maxY){ mpYear++; renderMpGrid(); }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    firebase.auth().signInWithPopup(provider)
      .then(result => {
        ...
        try{ localStorage.setItem('vikram_gcal_user', gcalUser); }catch(e){}
        resolve(token);
      })
      .catch(reject);
  });
...
function generateICS(scope){
  ...
  if(scope==='all' || scope==='holidays'){
    Object.entries(HOL).forEach(([key,h])=>{
      ...
      lines.push('BEGIN:VEVENT',
        `UID:vikram-hol-${key}@vikram.app`,`DTSTAMP:${stamp}`,
        `DTSTART;VALUE=DATE:${ds}`,`DTEND;VALUE=DATE:${de}`,
        `SUMMARY:🇳🇵 ${icsEscape(h.en)}`,`DESCRIPTION:${icsEscape(h.np)}`,
  ...
  if(scope==='all' || scope==='events'){
    userEvents.forEach((ev,i)=>{
      ...
      lines.push('BEGIN:VEVENT',
        `UID:vikram-evt-${ev.adMs}-${i}@vikram.app`,`DTSTAMP:${stamp}`,
        `DTSTART;VALUE=DATE:${ds}`,`DTEND;VALUE=DATE:${de}`,
...
window.addEventToCalendar=function(adMs){
  ...
  if(!ev){ if(typeof vikramToast==='function') vikramToast('⚠️ Event not found'); return; }
  ...
  a.href=url; a.download=`${safeName}.ics`; a.click();
  URL.revokeObjectURL(url);
  if(typeof vikramToast==='function') vikramToast('📅 Event saved — open the file to add it to your calendar');
};
function exportICS(scope){
  ...
  a.href=url; a.download=`vikram-${nameBit}-${vY}.ics`; a.click();
  URL.revokeObjectURL(url);
  renderGcalCard();
}
...
window.copyICSToClipboard=function(scope){
  ...
  try{
    navigator.clipboard.writeText(content)
      .then(()=>{ if(typeof vikramToast==='function') vikramToast('📋 .ics content copied'); })
      .catch(()=>{ if(typeof vikramToast==='function') vikramToast('⚠️ Could not copy'); });
  }catch(e){ if(typeof vikramToast==='function') vikramToast('⚠️ Clipboard not available'); }
};
...
async function gcalConnect(){
  if(!window.firebase || !firebase.auth){
    if(typeof vikramToast==='function') vikramToast('Google sign-in is unavailable in this build.');
    return;
  }
  ...
  if(!fbUser){
    ...
    if(typeof vikramToast==='function') vikramToast('Please sign in with Google first.');
    try{
      await new Promise<void>((res,rej)=>{
        ...
        firebase.auth().signInWithPopup(provider).then(r=>{
          ...
          if(tok){
            gcalToken=tok; gcalTokenExp=Date.now()+3600000;
            gcalUser=r.user.email||r.user.displayName||'Connected';
            try{ localStorage.setItem('vikram_gcal_user',gcalUser); }catch(e){}
            // update profile UI too
            if(typeof updateGoogleUI==='function') updateGoogleUI(r.user);
          }
          res();
        }).catch(rej);
      });
      ...
      if(typeof vikramToast==='function') vikramToast('❌ Sign-in cancelled.');
      return;
    }
  } else {
    ...
    try{
      ...
      if(typeof vikramToast==='function') vikramToast('❌ Could not get calendar access. Try again.');
      return;
    }
  }
  renderGcalCard();
  if(typeof vikramToast==='function') vikramToast('✅ Connected to Google Calendar!');
}

function gcalDisconnect(){
  gcalToken=null; gcalTokenExp=0; gcalUser=null;
  localStorage.removeItem('vikram_gcal_user');
  renderGcalCard();
  if(typeof vikramToast==='function') vikramToast('Disconnected from Google Calendar.');
}
...
async function gcalPushEvents(prog){
  ...
  for(let i=0;i<toPush.length;i++){
    ...
    if(prog) prog.textContent=`Pushing ${i+1}/${total}…`;
    try{
...
async function gcalPullEvents(prog){
  ...
  try{
    do{
      ...
      if(prog) prog.textContent='Pulling events from Google…';
      ...
      for(const item of (data.items||[])){
        ...
        if(endStrRaw){
          ...
          if(item.end.date) endCandidate=new Date(endCandidate.getTime()-86400000);
          if(endCandidate.getTime()>=startAd.getTime()) endAd=endCandidate;
        }
...
async function gcalSyncNow(){
  ...
  if(!gcalToken||gcalIsExpired()){
    await gcalConnect();
    if(!gcalToken) return; // connect was cancelled
  }
  gcalSyncing=true;
  ...
  try{ localStorage.setItem('vikram_gcal_last_sync', String(gcalLastSync)); }catch(e){}

  const parts=[];
  if(pushRes.total>0) parts.push(`${pushRes.done} sent`+(pushRes.failed?`, ${pushRes.failed} failed`:''));
  if(pullRes.imported>0) parts.push(`${pullRes.imported} imported`);
  ...
  if(pushRes.failed>0 && pushRes.done===0 && pushRes.total>0){
    ...
    if(typeof vikramToast==='function') vikramToast(msg); else alert(msg);
  }
    ...
    ${isConn
      ...
      : `<div class="gcal-status"><div class="gcal-dot grey"></div>Not connected</div>
         ...
         <div class="gcal-block-sub">${fbUser
           ...
           : `Sign in with your Google account to sync events both ways with Google Calendar — no setup needed.`
         }</div>
...
let cycleData = (()=>{
  try{ return JSON.parse(localStorage.getItem('vikram_cycle')||'{}'); }
  catch(e){ return {}; }
})();

function saveCycle(){
  try{ localStorage.setItem('vikram_cycle', JSON.stringify(cycleData)); }catch(e){
    if(e&&(e.name==='QuotaExceededError'||e.code===22||e.code===1014)){
      _lsPrune();
      try{ localStorage.setItem('vikram_cycle', JSON.stringify(cycleData)); }catch(e2){}
    }
  }
  if(typeof window._pairOnCycleSave === 'function') window._pairOnCycleSave();
}
...
function getPhaseForDate(ts){
  if(!cycleData.lastPeriodStart || ts == null) return null;
  const cl = cycleData.cycleLength || 28, pl = cycleData.periodLength || 5;
  const ovDay = cl - 14;
  const follEnd = ovDay - 3;
  const ovEnd = ovDay + 2;
  ...
  if(diffDays < -3650 || diffDays > 3650) return null; // sanity guard
  let day = ((diffDays % cl) + cl) % cl + 1; // 1-indexed day within a cl-length cycle

  if(day <= pl)      return { name:'Menstrual',  dayIn:day };
  if(day <= follEnd) return { name:'Follicular', dayIn:day-pl };
  if(day <= ovEnd)   return { name:'Ovulation',  dayIn:day-(ovDay-2)+1 };
...
function openSymptomLogSheet(dateTs){
  ...
  const existing = (cycleData.symptomLogs||[]).find(s=>{
    const sd = new Date(s.date); const sm = new Date(sd.getFullYear(),sd.getMonth(),sd.getDate()).getTime();
...
function symLogToggleSymptom(id){
  if(typeof haptic === 'function') haptic('light');
  if(_symLogSelectedSymptoms.has(id)) _symLogSelectedSymptoms.delete(id);
  else _symLogSelectedSymptoms.add(id);
  renderSymptomLogSheet();
}

function saveSymptomLog(){
  if(!_symLogSelectedMood && _symLogSelectedSymptoms.size === 0){
    if(typeof vikramToast==='function') vikramToast('⚠️ Pick a mood or at least one symptom');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  if(idx >= 0) cycleData.symptomLogs[idx] = entry;
  else cycleData.symptomLogs.push(entry);
  cycleData.symptomLogs.sort((a,b)=>a.date-b.date);
  saveCycle();
  closeSymptomLogSheet();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Logged — thanks for checking in 💭');
}
...
function renderSymptomHistorySheet(){
    ...
    ${[...logs].reverse().map((l, revIdx) => {
      ...
      return `<div class="symlog-row">
        ...
        ${(l.symptoms&&l.symptoms.length) ? `<div class="symlog-symptoms">${l.symptoms.map(sid=>{
          const sm = SYMPTOM_META[sid]; return sm ? `<span class="symlog-symptom-pill">${sm.emoji} ${sm.label}</span>` : '';
        }).join('')}</div>` : ''}
  ...
  if(sub) sub.textContent = logs.length + ' ' + (logs.length === 1 ? 'entry' : 'entries') + ' · tracked by phase';
}
...
function renderPhaseInsightsHTML(logs){
  ...
  logs.forEach(l=>{
    const ph = getPhaseForDate(l.date);
    if(ph && buckets[ph.name]) buckets[ph.name].push(l);
  });
...
function symDeleteLog(idx){
  ...
  if(desc) desc.textContent = 'Mood & symptom entry · ' + dateStr;
  document.getElementById('symDeleteLogOverlay').classList.add('open');
}
function symDeleteLogClose(e){
  if(e && e.target !== document.getElementById('symDeleteLogOverlay')) return;
  if(typeof haptic === 'function') haptic('light');
...
function symDeleteLogConfirm(){
  ...
  renderCycleSheet();
  if(typeof haptic === 'function') haptic('light');
  if(typeof vikramToast==='function') vikramToast('🗑 Entry deleted');
}
...
function saveIntercourseLog(){
  ...
  if(idx >= 0) cycleData.intercourseLogs[idx] = entry;
  else cycleData.intercourseLogs.push(entry);
  cycleData.intercourseLogs.sort((a,b)=>a.date-b.date);
  saveCycle();
  closeIntercourseLogSheet();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Intercourse logged 💕');
}
...
function saveOpkLog(){
  if(!_opkLogResult){
    if(typeof vikramToast==='function') vikramToast('⚠️ Pick a test result');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  if(idx >= 0) cycleData.opkLogs[idx] = entry;
  else cycleData.opkLogs.push(entry);
  ...
  if(_opkLogResult === 'pos'){
    const hasOvulToday = (cycleData.logs||[]).some(l=>{
      if(l.type!=='ovulation') return false;
      const ld = new Date(l.date); const lm = new Date(ld.getFullYear(),ld.getMonth(),ld.getDate()).getTime();
  ...
  if(typeof vikramToast==='function') vikramToast(msgs[_opkLogResult] || '✅ Logged');
}
...
function saveMucusLog(){
  if(!_mucusLogType){
    if(typeof vikramToast==='function') vikramToast('⚠️ Pick a mucus type');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  if(idx >= 0) cycleData.mucusLogs[idx] = entry;
  else cycleData.mucusLogs.push(entry);
  cycleData.mucusLogs.sort((a,b)=>a.date-b.date);
  saveCycle();
  closeMucusLogSheet();
  renderCycleSheet();
  const mm = MUCUS_TYPES[_mucusLogType];
  if(typeof vikramToast==='function') vikramToast(`✅ ${mm.label} mucus logged`);
}
...
function contraPackDayInfo(){
  ...
  const start = new Date(c.startDate); const startMid = new Date(start.getFullYear(),start.getMonth(),start.getDate()).getTime();
...
function contraTakenToday(){
  ...
  return (cycleData.contraceptionLogs||[]).some(l=>{
    const ld = new Date(l.date); return new Date(ld.getFullYear(),ld.getMonth(),ld.getDate()).getTime()===today && l.taken;
  });
}
function openContraSetupSheet(){
  ...
  if(typeEl) typeEl.value = c.type || 'pill';
  const plEl = document.getElementById('contraPackLenInput');
  if(plEl) plEl.value = c.packLength || 21;
  const phEl = document.getElementById('contraPlaceboLenInput');
  if(phEl) phEl.value = c.placeboLength != null ? c.placeboLength : 7;
  const sdEl = document.getElementById('contraStartDateInput');
  if(sdEl) sdEl.value = c.startDate ? new Date(c.startDate).toISOString().slice(0,10) : '';
  const rtEl = document.getElementById('contraReminderTimeInput');
  if(rtEl) rtEl.value = c.reminderTime || '08:00';
...
function saveContraSetup(){
  ...
  if(!startRaw){
    if(typeof vikramToast==='function') vikramToast('⚠️ Pick a pack start date');
    return;
  }
  ...
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Birth control tracking set up');
}
...
function contraMarkTaken(){
  ...
  const idx = cycleData.contraceptionLogs.findIndex(l=>{
    const ld = new Date(l.date); return new Date(ld.getFullYear(),ld.getMonth(),ld.getDate()).getTime()===today;
  });
  if(idx >= 0) cycleData.contraceptionLogs[idx].taken = !cycleData.contraceptionLogs[idx].taken;
  else cycleData.contraceptionLogs.push({ date: Date.now(), taken: true });
  saveCycle();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast(contraTakenToday() ? '✅ Marked taken' : '↩️ Unmarked');
}
function contraRemove(){
  ...
  saveCycle();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('🗑 Birth control tracking removed');
}
...
function openBbtLogSheet(dateTs){
  ...
  if(tempEl) tempEl.value = existing ? existing.temp : '';
  const timeEl = document.getElementById('bbtTimeInput');
  if(timeEl) timeEl.value = existing && existing.time ? existing.time : '07:00';
...
function saveBbtLog(){
  ...
  if(!raw || isNaN(temp)){
    if(typeof vikramToast==='function') vikramToast('⚠️ Enter a temperature');
  ...
  if(temp < min || temp > max){
    if(typeof vikramToast==='function') vikramToast('⚠️ That temperature looks out of range');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  if(idx >= 0) cycleData.bbtLogs[idx] = entry;
  else cycleData.bbtLogs.push(entry);
  cycleData.bbtLogs.sort((a,b)=>a.date-b.date);
  saveCycle();
  closeBbtLogSheet();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Temperature logged 🌡️');
}

// ── Convert a BBT entry's stored temp to the currently-displayed unit ──
function _bbtToUnit(entry, displayUnit){
  if(entry.unit === displayUnit) return entry.temp;
  if(entry.unit === 'C' && displayUnit === 'F') return entry.temp * 9/5 + 32;
  if(entry.unit === 'F' && displayUnit === 'C') return (entry.temp - 32) * 5/9;
  return entry.temp;
}
...
function renderTtcBbtChart(logs){
  ...
  if(hi - lo < 0.4){ const mid=(hi+lo)/2; lo = mid-0.4; hi = mid+0.4; }
  lo -= 0.15; hi += 0.15;
  ...
  const ovulSet = new Set((cycleData.logs||[]).filter(l=>l.type==='ovulation').map(l=>{
    const d=new Date(l.date); return new Date(d.getFullYear(),d.getMonth(),d.getDate()).getTime();
  }));
  ...
  const xLabels = logs.map((l,i)=>{
    if(logs.length > 10 && i % Math.ceil(logs.length/8) !== 0 && i !== logs.length-1) return '';
  ...
  const mucusRow = logs.map((l,i)=>{
    ...
    const ml = (cycleData.mucusLogs||[]).find(m=>{
      const md = new Date(m.date); return new Date(md.getFullYear(),md.getMonth(),md.getDate()).getTime() === dMid;
    });
    if(!ml) return '';
    const x = xFor(i);
...
function medAddItem(){
  ...
  if(!name){
    if(typeof vikramToast==='function') vikramToast('⚠️ Enter a name');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  _medAddName = ''; _medAddDose = '';
  saveCycle();
  renderMedLogSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Added to your list');
}
...
function openVitalsLogSheet(dateTs){
  ...
  if(wEl) wEl.value = existing && existing.weight!=null ? existing.weight : '';
  const watEl = document.getElementById('vitalsWaterInput');
  if(watEl) watEl.value = existing && existing.water!=null ? existing.water : '';
...
function saveVitalsLog(){
  ...
  if(weight==null && water==null){
    if(typeof vikramToast==='function') vikramToast('⚠️ Enter weight or water intake');
    return;
  }
  if(weight!=null && isNaN(weight)){
    if(typeof vikramToast==='function') vikramToast('⚠️ That weight looks invalid');
    return;
  }
  if(typeof haptic === 'function') haptic('light');
  ...
  if(idx >= 0) cycleData.vitalsLogs[idx] = entry;
  else cycleData.vitalsLogs.push(entry);
  cycleData.vitalsLogs.sort((a,b)=>a.date-b.date);
  saveCycle();
  closeVitalsLogSheet();
  renderCycleSheet();
  if(typeof vikramToast==='function') vikramToast('✅ Logged 📋');
}
...
function renderVitalsHistorySheet(){
  ...
  body.innerHTML = `<div class="ssec" style="padding-top:4px;">
    ...
    ${[...logs].reverse().map((l, revIdx)=>{
      const realIdx = logs.length - 1 - revIdx;
      const parts = [];
      if(l.weight!=null) parts.push(`${l.weight}${l.weightUnit||'kg'}`);
      if(l.water!=null) parts.push(`${l.water}L water`);
      return `<div class="srow">
        <div style="min-width:0;">
...
function buildMonthReviewCard(notSet, cl, pl, dataSrc?){
  try {
    ...
    const orderedDow = [];
    for(let i=0;i<7;i++) orderedDow.push(dowLabels[(cfg.fd+i)%7]);
    let cells = '';
    for(let p=0;p<startDow;p++) cells += `<div></div>`;
    for(let d=1;d<=monthDays;d++){
      ...
      if(!notSet && src.lastPeriodStart){
        const baseTs = new Date(src.lastPeriodStart); baseTs.setHours(0,0,0,0);
        ...
        if(dayNum >= 1 && dayNum <= cl){
          ...
          if(dayNum >= 1 && dayNum <= effectivePl_)               { phaseBg='rgba(255,71,87,.75)';   phaseLabel='🩸'; textColor='#fff'; }
          else if(hasOvulLog)                                      { phaseBg='rgba(168,85,247,.75)';  phaseLabel='✨'; textColor='#fff'; }
          else if(dayNum >= fertileStart_ && dayNum <= ovDay2_)   { phaseBg='rgba(99,102,241,.65)';  textColor='#fff'; }
          else if(dayNum > ovDay2_)                                { phaseBg='rgba(33,150,243,.55)';  textColor='rgba(255,255,255,.9)'; }
          else                                                     { phaseBg='rgba(255,107,157,.45)'; }
        }
      }
...
function _applyCycleModeUI(){
  ...
  if(btn){
    ...
    if(mode==='ttc') btn.classList.add('mode-ttc');
    if(mode==='pregnant') btn.classList.add('mode-pregnant');
    if(mode==='avoid') btn.classList.add('mode-avoid');
  }
  if(icon) icon.textContent = meta.icon;
  if(lbl)  lbl.textContent  = meta.label;
  // Only set the title if Partner Mode isn't currently overriding it
  if(title && !(typeof _partnerModeActive !== 'undefined' && _partnerModeActive)) title.textContent = meta.title;
}
...
function savePregSetup(){
  ...
  if(type === 'lmp'){
    ...
    if(!val){ _pregFlashInvalid('pregSetupLmpInput'); return; }
    ...
    if(!val){ _pregFlashInvalid('pregSetupDueInput'); return; }
    const [y,m,dd] = val.split('-').map(Number);
...
function renderCycleSheet(){
  ...
  if(_partnerModeActive){
    ...
    if(body){
      try{ renderPartnerModeCycleSheet(body); }
      catch(e){
  ...
  if(getCycleMode() === 'pregnant'){
    const body = document.getElementById('cycleBody');
    if(body){
      try{ renderPregnancyDashboard(body); }
      catch(e){
  ...
  <div class="ttc-banner">
    ...
    <div class="ttc-banner-body">
      ...
      <div class="ttc-banner-sub">${
        inFertile ? 'You\'re in your fertile window — a great time to try.'
        ...
        : 'Track ovulation to pinpoint your next fertile window.'
      }</div>
    </div>
  </div>` : ''}
  ${getCycleMode() === 'ttc' && notSet ? `
  ...
  <div class="ttc-banner" style="background:linear-gradient(155deg,rgba(56,189,248,.12),rgba(14,165,233,.06));border-color:rgba(14,165,233,.22);">
    ...
    <div class="ttc-banner-body">
      ...
      <div class="ttc-banner-sub">${
        inFertile ? 'You\'re in your fertile window — use protection or abstain if avoiding pregnancy.'
        ...
        : 'Track ovulation to know exactly when your risk window opens.'
      }</div>
    </div>
  </div>` : ''}
  ${getCycleMode() === 'avoid' && notSet ? `
      ...
      <div class="cyc-log-btn-left">
        ...
        <div>
          ...
          <div class="ssub">${(()=>{
            ...
            if(todayLog && todayLog.flow) return 'Logged today · ' + todayLog.flow.charAt(0).toUpperCase()+todayLog.flow.slice(1)+' flow ✓';
            if(todayLog) return 'Logged today ✓';
        ...
        <div>
          ...
          <div class="ssub">${(() => {
            ...
            const loggedToday = (cycleData.intercourseLogs||[]).some(s=>{
              const sd = new Date(s.date); return new Date(sd.getFullYear(),sd.getMonth(),sd.getDate()).getTime() === todayMid;
            });
  ...
  <div class="ssec" style="padding-top:0;">
    ...
    ${(()=>{
      ...
      if(!prs.length) return `<div style="padding:12px 4px;font-size:12px;font-weight:700;color:var(--dsub);">No cycle reminders yet — add one to get notified at the right phase 🔔</div>`;
      return prs.map(r=>{
...
try{ _partnerModeActive = localStorage.getItem('vikram_partner_mode') === '1'; }catch(e){}

function _pmPairLoad(){
  try{ return JSON.parse(localStorage.getItem('vikram_pair_v1')||'{}'); }catch(e){ return {}; }
}
...
function togglePartnerMode(){
  ...
  try{ localStorage.setItem('vikram_partner_mode', _partnerModeActive ? '1' : '0'); }catch(e){}
  _applyPartnerModeUI();
  closeCycleModePicker();
  renderCycleSheet();
}
...
function _applyPartnerModeUI(){
  ...
  const title  = document.getElementById('cycleTitleLabel');
  if(row) row.classList.toggle('on', _partnerModeActive);
  if(sw)  sw.classList.toggle('on', _partnerModeActive);
  if(sub) sub.textContent = _partnerModeActive ? 'Showing your partner\'s synced cycle' : 'View your partner\'s synced cycle';
  if(title) title.textContent = _partnerModeActive ? '💑 Partner View' : '🌸 My Cycle';
}
...
function renderPartnerModeCycleSheet(body){
  ...
  if(!notSet){
    if(ovulLog){
      if(ovulDaysAgo <= 1){ phaseName='Ovulation'; phaseColor='#5a5a68'; }
      else                 { phaseName='Luteal';    phaseColor='#3B82F6'; }
    } else if(periodEndLog && day <= pl){
      phaseName='Follicular'; phaseColor='#FF6B9D';
    } else if(day <= pl)           { phaseName='Menstrual';  phaseColor='#FF4757'; }
    else if(day < ovDay - 2)       { phaseName='Follicular'; phaseColor='#FF6B9D'; }
    else if(day <= ovDay + 2)      { phaseName='Ovulation';  phaseColor='#5a5a68'; }
    else                           { phaseName='Luteal';     phaseColor='#3B82F6'; }
  ...
  const nextLabel = (() => {
    if(notSet) return '—';
    if(ovulLog){ const df = 14 - ovulDaysAgo; return df <= 0 ? 'Today' : df + 'd'; }
    return dNext === 0 ? 'Today' : dNext + 'd';
  })();
  ...
  setTimeout(function(){
    if(typeof window.renderPairMoments === 'function') window.renderPairMoments();
  }, 0);
}
...
function pmUnpair(){
  ...
  if(!confirm('Unpair from ' + partnerName + '? This only affects your device.')) return;
  ...
  try{
    ...
    delete pd.partnerCode; delete pd.partnerName; delete pd.partnerCycleData;
    delete pd.sharedAt; delete pd.partnerCycleUpdatedAt;
  ...
  try{ localStorage.setItem('vikram_partner_mode', '0'); }catch(e){}
  _applyPartnerModeUI();

  // Refresh the pair section (now shows unpaired state) and re-render cycle
  if(typeof renderPairSection === 'function') renderPairSection();
  renderCycleSheet();
  if(typeof vikramToast === 'function') vikramToast('💔 Unpaired from ' + partnerName);
}
...
function flowPickerSelect(level){
  ...
  if(saveBtn) saveBtn.textContent = 'Log as ' + level.charAt(0).toUpperCase() + level.slice(1);
}
function flowPickerClose(e){
  if(e && e.target !== document.getElementById('flowPickerOverlay')) return;
  _flowPickerSelected = null;
  document.getElementById('flowPickerOverlay').classList.remove('open');
}
function flowPickerSave(){
  if(typeof haptic === 'function') haptic('medium');
  if(typeof checkDateRollover === 'function') checkDateRollover(); // guard against a stale cached "today"
  const nowToday = getKathmanduToday();
  const today = new Date(nowToday.getFullYear(),nowToday.getMonth(),nowToday.getDate()).getTime();
  const entry: any = { type: _flowPickerType, date: today };
  if(_flowPickerSelected) entry.flow = _flowPickerSelected;
  ...
  if(typeof vikramToast === 'function') vikramToast('✅ ' + typeLabel + flowLabel);
  _flowPickerSelected = null;
}
...
function cyEditLogOpen(idx){
  ...
  if(inp){ inp.value = dateStr; inp.max = todayStr; }
...
function cyLogByDateOpen(){
  ...
  const titleEl = document.getElementById('cyLogByDateTitle');
  if(titleEl) titleEl.textContent = 'Log by date';
  const saveBtn = document.getElementById('cyLogByDateSaveBtn');
  ...
  const todayStr = t.getFullYear() + '-' + pad(t.getMonth()+1) + '-' + pad(t.getDate());
  const inp = document.getElementById('cyLogByDateInput');
  if(inp){ inp.value = todayStr; inp.max = todayStr; }
...
function cyLogByDateClose(e){
  if(e && e.target !== document.getElementById('cyLogByDateOverlay')) return;
...
function cyLogByDateSave(){
  const inp = document.getElementById('cyLogByDateInput');
  if(!inp || !inp.value){ vikramToast && vikramToast('⚠️ Please pick a date'); return; }
  ...
  if(isNaN(ts)){ vikramToast && vikramToast('⚠️ Invalid date'); return; }
  ...
  if(typeof vikramToast==='function') vikramToast(isEdit ? ('✅ Updated to ' + dateStr) : ('✅ ' + (labels[type]||type) + ' logged for ' + dateStr));
  _cyEditIdx = -1;
  const titleEl = document.getElementById('cyLogByDateTitle');
  if(titleEl) titleEl.textContent = 'Log by date';
  const saveBtn = document.getElementById('cyLogByDateSaveBtn');
...
function cyDeleteLog(idx){
  ...
  if(desc) desc.textContent = (labels[l.type]||l.type) + ' · ' + dateStr;
  document.getElementById('cyDeleteLogOverlay').classList.add('open');
}

function cyDeleteLogClose(e){
  if(e && e.target !== document.getElementById('cyDeleteLogOverlay')) return;
...
function cyDeleteLogConfirm(){
  ...
  if(document.getElementById('logHistorySheet')?.classList.contains('open')){
    renderLogHistorySheet();
    if(!(cycleData.logs||[]).length) closeLogHistorySheet();
  }
  const labels = {period_start:'Period start',period_end:'Period end',ovulation:'Ovulation'};
  if(typeof vikramToast==='function') vikramToast('🗑 ' + (labels[removed.type]||removed.type) + ' deleted');
}
function cyResetConfirmClose(e){
  if(e && e.target !== document.getElementById('cyResetConfirmOverlay')) return;
...
function cyResetConfirmYes(){
  ...
  try{ closeCycleSettingsSheet(); }catch(e){}
  cycleData = {};
...
(function(){
  ...
  function db(){
    try{ return (window.firebase && firebase.database) ? firebase.database() : null; }
    catch(e){ return null; }
  }

  function fbReady(){
    try{ return !!(window.firebase && firebase.apps && firebase.apps.length && firebase.database); }
  ...
  // ── local helpers ──────────────────────────────────────────────────────────
  function pairLoad(){ try{ return JSON.parse(localStorage.getItem(PAIR_KEY)||'{}'); }catch(e){ return {}; } }
  function pairSave(d){ try{ localStorage.setItem(PAIR_KEY, JSON.stringify(d)); }catch(e){} }
  ...
  function ensureAuth(){
    return new Promise((resolve, reject) => {
      if(!fbReady() || !firebase.auth) return reject(new Error('firebase_not_ready'));
      const user = firebase.auth().currentUser;
      if(user){ resolve(user); return; }
      // Don't create anonymous session if Google sign-in is in progress
      if(_googleSignInInProgress){ reject(new Error('google_signin_in_progress')); return; }
    ...
    ensureAuth().then(() => {
      database.ref('cycles/'+d.myCode).set(payload)
        ...
        .catch(e => {
          ...
          if(typeof vikramToast==='function') vikramToast('❌ Sync failed: ' + (e.code||'check Firebase rules'));
          if(typeof renderPairSection==='function') renderPairSection();
        });
  ...
  function fbSubscribePartnerCycle(partnerCode){
    if(!fbReady() || !partnerCode) return;
    const database = db();
    if(!database) return;
    ensureAuth().then(() => {
      if(_partnerCycleListener && _cycleRef){
        try{ _cycleRef.off('value', _partnerCycleListener); }catch(e){}
      }
  ...
  function fbSubscribeMsgs(myCode, partnerCode){
    ...
    ensureAuth().then(() => {
      if(_msgsListener && _msgsRef){
        try{ _msgsRef.off('value', _msgsListener); }catch(e){}
      }
      _msgsRef = database.ref('moments/'+roomKey(myCode, partnerCode));
      _msgsListener = _msgsRef.on('value', snap => {
        const val: any = snap.val() || {};
        const msgs = Object.values(val).sort((a: any,b: any) => a.ts - b.ts);
        try{ localStorage.setItem('vikram_pair_moments', JSON.stringify(msgs)); }catch(e){}
        renderMoments();
      }, e => {
  ...
  function fbSendMsg(myCode, partnerCode, text){
    ...
    return ensureAuth().then(() => {
      ...
      if(typeof vikramToast==='function') vikramToast('❌ Message failed: ' + (e.code||'check Firebase rules'));
      return false;
    });
  }
  ...
  function fbDetach(){
    try{
      if(_msgsListener && _msgsRef) _msgsRef.off('value', _msgsListener);
      if(_partnerCycleListener && _cycleRef) _cycleRef.off('value', _partnerCycleListener);
    }catch(e){}
    _msgsListener = null; _partnerCycleListener = null;
    _msgsRef = null; _cycleRef = null;
  }
  ...
  function myCode(){
    let d=pairLoad();
    if(!d.myCode){ d.myCode=genCode(); pairSave(d); }
    return d.myCode;
  }
  ...
  function partnerPhaseInfo(p){
    ...
    const last=new Date(p.lastPeriodStart); last.setHours(0,0,0,0);
    ...
    if(day<=pl){phase='Period';phaseColor='#FF4757';}
    else if(day<ovDay-2){phase='Follicular';phaseColor='#FF6B9D';}
    else if(day<=ovDay+2){phase='Ovulation';phaseColor='#5a5a68';}
    else{phase='Luteal';phaseColor='#3B82F6';}
  ...
  window.renderPairSection = function(){
    ...
    const isOnline = paired && fbOk && !permDenied;
    if(entrySub){
      if(paired) entrySub.textContent = 'Connected · live cycle sync';
      else if(fbOk) entrySub.textContent = 'Real-time sync ready';
      else entrySub.textContent = 'Connect with your partner\'s cycle';
    }
    if(entryDot){
    ...
    if(paired && typeof window.renderPairMoments === 'function'){
      ...
      if(sheetOpen) setTimeout(window.renderPairMoments, 0);
    }
  };
  ...
  window.pairCopyCode = function(code){
    try{
      navigator.clipboard.writeText(code).then(()=>{
        ...
        if(el){ const orig=el.textContent; el.textContent='Copied!'; el.style.color='#22c55e'; setTimeout(()=>{ el.textContent=orig; el.style.color=''; },1500); }
      });
    }catch(e){}
  };
  ...
  window.pairConnect = function(){
    ...
    if(raw.length < 6){ inp.style.borderColor='#f43f5e'; setTimeout(()=>inp.style.borderColor='',1200); return; }
    const me = myCode().replace('-','');
    if(raw === me){ inp.style.borderColor='#f43f5e'; if(typeof vikramToast==='function') vikramToast('⚠️ That\'s your own code!'); setTimeout(()=>inp.style.borderColor='',1200); return; }
  ...
  window.pairConnectConfirm = function(){
    ...
    if(fbReady()){
      ...
      if(typeof vikramToast === 'function') vikramToast('💞 Connected — syncing live!');
    } else {
      // Firebase still loading — retry once it is ready
      if(typeof vikramToast === 'function') vikramToast('💞 Connected — will sync when ready');
      var _retryCode = code;
      ...
      var _retryInterval = setInterval(function(){
        ...
        if(fbReady()){
          ...
          if(typeof vikramToast === 'function') vikramToast('⚠️ Sync service unavailable — saved locally');
        }
      }, 500);
    }
    renderPairSection();
  ...
  window.pairDisconnect = function(){
    if(!confirm('Disconnect from your partner? This only affects your device.')) return;
    if(typeof haptic === 'function') haptic('medium');
    fbDetach();
    const d = pairLoad();
    delete d.partnerCode; delete d.partnerName; delete d.partnerCycleData; delete d.sharedAt; delete d.partnerCycleUpdatedAt;
    pairSave(d);
    // Clear local message cache
    try{ localStorage.removeItem('vikram_pair_moments'); }catch(e){}
    renderPairSection();
  };
  ...
  function msgsLoad(){ try{ return JSON.parse(localStorage.getItem(MSG_KEY)||'[]'); }catch(e){ return []; } }
  ...
  function renderMoments(){
    ...
    if(pmSub) pmSub.textContent = msgs.length ? msgs.length + (msgs.length===1?' message':' messages') : 'Just between you two';
  ...
  function doSendMoment(text){
    ...
    if(typeof haptic === 'function') haptic('medium');
    const d = pairLoad();
    if(!d.partnerCode){ if(typeof vikramToast==='function') vikramToast('Connect with a partner first 💞'); return; }
    ...
    fbSendMsg(d.myCode || myCode(), d.partnerCode, text).then(sent => {
      if(!sent) if(typeof vikramToast==='function') vikramToast('⚠️ Saved locally — sync when back online');
    });
  }
...
function openPartnerMomentsSheet(){
  ...
  if(inp){
    ...
    try{
      ...
      if(d.partnerName) pname = d.partnerName;
    }catch(e){}
...
function renderLogHistorySheet(){
  ...
  if(sub) sub.textContent = logs.length + ' ' + (logs.length === 1 ? 'entry' : 'entries');
}
...
function cyDeleteLogFromSheet(idx){
  ...
  const dateStr = new Date(l.date).toLocaleDateString('en-NP',{month:'long',day:'numeric',year:'numeric'});
  const desc = document.getElementById('cyDeleteLogDesc');
  if(desc) desc.textContent = (logLabels[l.type]||l.type) + ' · ' + dateStr;
  document.getElementById('cyDeleteLogOverlay').classList.add('open');
}
...
function renderCycleSettingsSheet(){
  ...
  if(sub) sub.textContent = 'Cycle ' + cl + 'd · Period ' + pl + 'd';
}
...
function renderCycleDetailSheet(){
  ...
  const nextLabel = (() => {
    if(ovulLog){ const dFromOvul = 14 - ovulDaysAgo; return dFromOvul <= 0 ? 'Today' : dFromOvul + 'd'; }
    return dNext === 0 ? 'Today' : dNext + 'd';
  })();
...
(function(){
  ...
  if(origOvHandler){
    ...
    origOvHandler.setAttribute('onclick',
...
function getCycleRegularityAlert(){
  ...
  if(logs.length < 4) return null; // need at least 4 starts → 3 cycle lengths
  const allCycles = [];
  for(let i = 1; i < logs.length; i++){
    const len = Math.round((logs[i].date - logs[i-1].date) / 86400000);
    if(len > 0 && len < 60) allCycles.push(len);
  }
  if(allCycles.length < 3) return null;
...
function renderCycleInsights(){
  ...
  if(!windowCycles.length) windowCycles = allCycles.slice(-3); // fallback if all data is older
  ...
  if(n >= 2){
    ...
    if(diff >= 1.5){ trendCls='up'; trendArrow='↗'; trendLabel = `+${Math.round(diff)}d vs prior avg`; }
    else if(diff <= -1.5){ trendCls='down'; trendArrow='↘'; trendLabel = `${Math.round(diff)}d vs prior avg`; }
    else { trendCls='flat'; trendArrow='→'; trendLabel = 'In line with average'; }
  }
  ...
  if(stdDev > 5 || range > 9){ regCls = 'irreg'; regLabel = 'Irregular'; }
  else if(stdDev > 2.5 || range > 5){ regCls = 'mod'; regLabel = 'Somewhat variable'; }
...
function exportCycleData(){
  ...
  if(cycleData.lastPeriodStart) rows.push(['Last period start', new Date(cycleData.lastPeriodStart).toLocaleDateString('en-NP')]);
  ...
  if(typeof vikramToast==='function') vikramToast('📤 Cycle data exported');
}
...
function renderClinicSummarySheet(){
  ...
  for(let i = 1; i < logs.length; i++){
    const len = Math.round((logs[i].date - logs[i-1].date) / 86400000);
    if(len > 0 && len < 60) cycles.push({ len, startDate: logs[i-1].date });
  }
...
function exportClinicSummaryPDF(){
  ...
  if(!d){
    if(typeof vikramToast==='function') vikramToast('⚠️ Open the summary first');
    return;
  }
...
function _renderCalendarToCanvas(canvas, PX){
  ...
  hGrad.addColorStop(0,'#1a1a1a');hGrad.addColorStop(1,'#000000');
  ctx.fillStyle=hGrad;
...
setInterval(function(){ if(typeof checkHabitReminders==='function') checkHabitReminders(); }, 60000);
  ...
  window.triggerInstall=function(){
    if(deferredPrompt){
      ...
      deferredPrompt.userChoice.then(r=>{
        if(r.outcome==='accepted') try{ localStorage.setItem('vikram_installed','1'); }catch(e){}
        deferredPrompt=null;
        window._vikramDeferredPrompt=null;
      });
    }
...
window.renderColorMatcherSheet=function(){
  ...
  if(!document.getElementById('cmFonts')){
    ...
    lk.id='cmFonts';lk.rel='stylesheet';
  ...
  s.running=false;s.score=0;s.timeLeft=60;s.streak=0;s.bestStreak=0;s.totalCorrect=0;s.locked=false;s.optionCount=6;
...
function cmStartGame(){
  const s=window.cmState;
  s.score=0;s.timeLeft=60;s.streak=0;s.bestStreak=0;s.totalCorrect=0;
  s.locked=false;s.running=true;s.optionCount=6;
  cmUpdateHUD();
  const ov=cmGet('cm-overlay');if(ov)ov.classList.add('hidden');
  const ring=cmGet('cm-ring');if(ring)ring.classList.add('active');
  const prompt=cmGet('cm-prompt');if(prompt)prompt.textContent='— pick the matching color —';
  // reset overlay tags/stats for fresh start
  const tags=cmGet('cm-ov-tags');if(tags)tags.style.display='flex';
  const stats=cmGet('cm-ov-stats');if(stats)stats.style.display='none';
  cmNextRound();
  clearInterval(s.timerInterval);
  s.timerInterval=setInterval(()=>{
    s.timeLeft--;
    const t=cmGet('cm-timer');if(t){t.textContent=s.timeLeft;t.classList.toggle('warning',s.timeLeft<=10);}
    if(s.timeLeft<=0)cmEndGame();
  },1000);
}
...
function cmHandleClick(color,btn){
  ...
  if(color.name===s.target.name){
    let earned=10;if(quick)earned+=5;
    s.streak++;s.totalCorrect++;if(s.streak>s.bestStreak)s.bestStreak=s.streak;
    if(s.streak%5===0){earned+=25;cmShowCombo(`🔥 ${s.streak}× STREAK +25`);}
    s.score=Math.max(0,s.score+earned);
    cmUpdateHUD();cmScorePop(btn,`+${earned}`,true);
    btn.classList.add('cm-flash-ok');
    const circle=cmGet('cm-circle');if(circle){circle.classList.add('cm-correct');setTimeout(()=>{circle.classList.remove('cm-correct');},400);}
    setTimeout(()=>btn.classList.remove('cm-flash-ok'),400);
    const scoreEl=cmGet('cm-score');
    if(scoreEl){scoreEl.style.transform='scale(1.35)';setTimeout(()=>scoreEl.style.transform='',250);}
    if(s.score>=200&&s.optionCount>4)s.optionCount=4;
    else if(s.score>=100&&s.optionCount>5)s.optionCount=5;
    setTimeout(cmNextRound,80);
  } else {
    s.locked=true;s.streak=0;s.score=Math.max(0,s.score-5);
    cmUpdateHUD();cmScorePop(btn,'-5',false);
    btn.classList.add('cm-flash-no');
    const circle=cmGet('cm-circle');if(circle){circle.classList.add('cm-wrong');setTimeout(()=>{circle.classList.remove('cm-wrong');},450);}
...
function cmUpdateHUD(){
  const s=window.cmState;
  const sc=cmGet('cm-score');if(sc)sc.textContent=s.score;
  const st=cmGet('cm-streak');if(st)st.textContent=s.streak;
}
...
function cmShowCombo(msg){
  const el=cmGet('cm-combo');if(!el)return;
...
function cmEndGame(){
  const s=window.cmState;
  s.running=false;clearInterval(s.timerInterval);
  const timer=cmGet('cm-timer');if(timer)timer.classList.remove('warning');
  const ring=cmGet('cm-ring');if(ring)ring.classList.remove('active');
  cmGet('cm-grid')?.querySelectorAll('button').forEach(b=>b.disabled=true);
  if(s.score>0)cmConfetti();
  setTimeout(()=>{
    const title=cmGet('cm-ov-title');const sub=cmGet('cm-ov-sub');
    const tags=cmGet('cm-ov-tags');const stats=cmGet('cm-ov-stats');
    const btn=cmGet('cm-start-btn');const ov=cmGet('cm-overlay');
    if(title)title.textContent=s.score>=150?'LEGENDARY':s.score>=80?'GREAT JOB':'GAME OVER';
    if(sub)sub.textContent=s.score>=150?'You are a color master!':s.score>=80?'Solid performance!':'Better luck next time';
    if(tags)tags.style.display='none';
    if(stats){stats.style.display='flex';
      const fs=cmGet('cm-final-score');const fc=cmGet('cm-final-correct');const fst=cmGet('cm-final-streak');
      if(fs)fs.textContent=s.score;if(fc)fc.textContent=s.totalCorrect;if(fst)fst.textContent=s.bestStreak;
    }
...
window.renderEchoSheet = function(targetEl) {
  ...
  let sealed = null;
  try { sealed = JSON.parse(localStorage.getItem(ECHO_STORAGE_KEY)); } catch {}
  ...
  if (sealed) { renderEchoSealed(inner, sealed); }
  else { renderEchoIntro(inner); }
};
...
function renderEchoQuestion(container) {
  ...
  ta?.addEventListener('input', () => {
    ...
    if (cnt) { cnt.textContent = `${l} / 280`; cnt.className = 'echo-count'+(l > 238 ? ' warn' : ''); }
    if (btn) btn.style.display = l > 20 ? 'block' : 'none';
  });
  btn?.addEventListener('click', () => { const a = ta?.value?.trim(); if (a) renderEchoTrigger(container, a); });

  const iv = setInterval(() => {
    i++;
    if (qEl) qEl.textContent = question.slice(0, i);
    if (i >= question.length) {
      clearInterval(iv);
      const w = document.getElementById('echoInputWrap');
      if (w) { w.style.display = 'flex'; w.style.flexDirection = 'column'; }
...
function renderEchoTrigger(container, answer) {
  ...
  function updatePreview() {
    ...
    let can=false, desc='';
    if (triggerType==='date' && dateVal) { can=true; desc=`on ${new Date(dateVal+'T12:00:00').toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}`; }
    if (pr) pr.style.display = can ? 'block' : 'none';
    if (pv) pv.textContent = desc;
  }
  ...
  container.addEventListener('click', function onSeal(e) {
    if (e.target.id === 'echoSealBtn') {
      ...
      try { localStorage.setItem(ECHO_STORAGE_KEY, JSON.stringify(data)); } catch {}
...
function renderEchoSealing(container, data) {
  ...
  let i = 0;
  if (els[0]) els[0].className='echo-sealing-step active';
  const iv = setInterval(() => {
    if (els[i]) els[i].className='echo-sealing-step done';
    i++;
    if (i < els.length) { if(els[i]) els[i].className='echo-sealing-step active'; }
    if (i >= steps.length) { clearInterval(iv); setTimeout(()=>renderEchoSealed(container, data), 1050); }
  }, 680);
}

function renderEchoSealed(container, data) {
  function getTriggerText() {
    if (data.triggerType==='date') return `on ${new Date(data.trigger.date+'T12:00:00').toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}`;
    if (data.triggerType==='age') return `when you turn ${data.trigger.age}`;
    if (data.triggerType==='event') return esc(data.trigger.event.toLowerCase());
...
function pomoLoadStats(){
  try{
    ...
    return d;
  }catch(e){ return {date:new Date().toDateString(),sessions:0,breakCount:0,minutes:0,dotLog:[]}; }
}
function pomoSaveStats(stats){ try{ localStorage.setItem(POMO_STATS_KEY, JSON.stringify(stats)); }catch(e){} }

function pomoLoadSettings(){
  try{
    const d = JSON.parse(localStorage.getItem(POMO_KEY)||'{}');
    if(d.workMins)  window.pomoState.workMins  = d.workMins;
    if(d.breakMins) window.pomoState.breakMins = d.breakMins;
    if(d.longBreakMins) window.pomoState.longBreakMins = d.longBreakMins;
  }catch(e){}
}
...
// ── Todos ──────────────────────────────────────────────────────────────────
let pomoTodos = [];
function pomoLoadTodos(){ try{ pomoTodos = JSON.parse(localStorage.getItem(POMO_TODOS_KEY)||'[]'); }catch(e){ pomoTodos=[]; } }
function pomoSaveTodos(){ try{ localStorage.setItem(POMO_TODOS_KEY, JSON.stringify(pomoTodos)); }catch(e){} }
...
function pomoTick(){
  ...
  s.secondsLeft--;
  pomoUpdateRing();
  if(s.secondsLeft <= 0){ pomoPhaseComplete(); }
}
...
function pomoTotalSec(){
  const s = window.pomoState;
  if(s.phase==='focus') return s.workMins*60;
  if(s.phase==='break') return s.breakMins*60;
...
function pomoUpdateRing(){
  ...
  const stats = pomoLoadStats();
  if(sessEl) sessEl.textContent = `${stats.sessions} session${stats.sessions!==1?'s':''} today`;
}
...
function pomoUpdateDash(){
  ...
  if(focusEl) focusEl.textContent = stats.sessions;
  if(minsEl)  minsEl.textContent  = stats.minutes;
  if(breakEl) breakEl.textContent = stats.breakCount;
  if(dotsEl){
    const maxDots = 12;
    const dots = stats.dotLog.slice(-maxDots);
    dotsEl.innerHTML = Array.from({length:maxDots},(_,i)=>{
      const d = dots[i];
      if(!d) return `<div class="pomo-dot"></div>`;
      if(d==='focus') return `<div class="pomo-dot done" title="Focus session"></div>`;
...
function pomoShowToast(msg){
  ...
  if(!t){ t=document.createElement('div'); t.id='pomoToast'; t.className='pomo-toast'; document.body.appendChild(t); }
  t.textContent = msg;
...
function pomoAdjust(key, delta){
  ...
  if(!s.running){
    if(key==='workMins' && s.phase==='focus') s.secondsLeft = s.workMins*60;
    if(key==='breakMins' && s.phase==='break') s.secondsLeft = s.breakMins*60;
    if(key==='longBreakMins' && s.phase==='longbreak') s.secondsLeft = s.longBreakMins*60;
    pomoUpdateRing();
  }
  const sub = document.getElementById('pomoSubtitle');
  if(sub) sub.textContent = `${s.workMins} min focus · ${s.breakMins} min rest`;
}
...
function pomoToggleTodo(id){
  ...
  if(t){ t.done = !t.done; pomoSaveTodos(); renderPomodoroTodos(); }
}
...
window.renderPomodoroSheet = function(){
    ...
    <div class="pomo-dash">
      ...
      <div class="pomo-session-dots" id="pomoDashDots">
        ${Array.from({length:12},(_,i)=>{
          ...
          if(i < 12 - Math.min(12,stats.dotLog.length)) return `<div class="pomo-dot"></div>`;
          if(d==='focus') return `<div class="pomo-dot done" title="Focus"></div>`;
          return `<div class="pomo-dot break" title="Break"></div>`;
        }).join('')}
      </div>
    </div>
  ...
  if(s.running){ pomoUpdateControls(); }
};
...
(function(){
  ...
  function bsYearOpts(sel){ let s=''; for(let y=2055;y<=2086;y++) s+=`<option value="${y}"${y===sel?' selected':''}>${y}</option>`; return s; }
  function bsMonthOpts(sel){ let s=''; MONTHS_EN.forEach((m,i)=>s+=`<option value="${i+1}"${i+1===sel?' selected':''}>${m} (${MONTHS_NE[i]})</option>`); return s; }
  function bsDayOpts(y,m,sel){ const max=BS[y]?.[m-1]??30; let s=''; for(let d=1;d<=max;d++) s+=`<option value="${d}"${d===sel?' selected':''}>${d}</option>`; return s; }
  function adYearOpts(sel){ let s=''; for(let y=1998;y<=2030;y++) s+=`<option value="${y}"${y===sel?' selected':''}>${y}</option>`; return s; }
  function adMonthOpts(sel){ let s=''; ADMON.forEach((m,i)=>s+=`<option value="${i+1}"${i+1===sel?' selected':''}>${m}</option>`); return s; }
  function adDayOpts(y,m,sel){ const max=new Date(y,m,0).getDate(); let s=''; for(let d=1;d<=max;d++) s+=`<option value="${d}"${d===sel?' selected':''}>${d}</option>`; return s; }
  ...
  function doConvertBsToAd(){
    ...
    const m=+document.getElementById('cvBsM').value;
    const d=+document.getElementById('cvBsD').value;
    if(!BS[y]){ document.getElementById('cvBsResult').innerHTML='<span style="color:var(--wknd)">Year out of range</span>'; return; }
    const max=BS[y][m-1];
    if(d<1||d>max){ document.getElementById('cvBsResult').innerHTML=`<span style="color:var(--wknd)">Day must be 1–${max}</span>`; return; }
    const ad=bsToAd(y,m,d);
  ...
  function doConvertAdToBs(){
    ...
    let bs;
    try{ bs=adToBs(ad); } catch(e){ document.getElementById('cvAdResult').innerHTML='<span style="color:var(--wknd)">Out of range</span>'; return; }
    if(!BS[bs.y]||bs.y<2055||bs.y>2086){ document.getElementById('cvAdResult').innerHTML='<span style="color:var(--wknd)">Corresponding BS date out of range (2055–2086)</span>'; return; }
...
<div class="cv-section">
  ...
  <div id="ageResult" style="display:none;border-radius:14px;background:rgba(130,80,200,.09);border:1px solid rgba(130,80,200,.18);padding:12px 14px;margin-top:12px;">
    ...
    if(window.initAgeCalc) window.initAgeCalc();
  };
})();
...
(function() {
  function initAgeCalc() {
    ...
    for (let y = maxY; y >= minY; y--) {
      ...
      o.value = String(y); o.textContent = y + ' BS';
      yEl.appendChild(o);
    }
...
function resetHomeSubViews(){
  ...
  if(typeof weekViewActive !== 'undefined' && weekViewActive){
    weekViewActive = false;
    if(tabWeek) tabWeek.classList.remove('on');
    if(typeof wvStopNowLineTimer==='function') wvStopNowLineTimer();
  }
  if(typeof dayViewActive !== 'undefined' && dayViewActive){
    dayViewActive = false;
...
function switchTab(tab) {
  if(!window._vikramRestoring) haptic('light');
  try { window.localStorage.setItem('vikram_active_tab', tab); } catch(e) {}
  if(tab !== 'notes' && typeof window.notesFlushSave === 'function') window.notesFlushSave();
  if(tab !== 'notes' && typeof window.tasksFlushSave === 'function') window.tasksFlushSave();
  resetHomeSubViews();
  ...
  if(schedFab) schedFab.style.display = (tab === 'schedule') ? '' : 'none';
  const schedAddFab = document.getElementById('schedAddFab');
  if(schedAddFab) schedAddFab.style.display = (tab === 'schedule') ? '' : 'none';

  if(tab === 'home') {
    // In home, show ccard unless 3M view is active
    if(tabAll)       tabAll.classList.add('on');
    if(mm3Active){
      if(mainContent) mainContent.style.display = 'none';
      const mmView = document.getElementById('mmView');
      if(mmView) mmView.style.display = 'block';
    } else {
    ...
    if(dcView)      dcView.classList.remove('active');
    if(cycleView)   cycleView.classList.remove('active');
    if(notesView)   notesView.classList.remove('active');
    if(schedView)   schedView.style.display   = 'none';
    if(settingsView)settingsView.classList.remove('active');
    if(profileView) profileView.classList.remove('active');
    scheduleActive = false;
    if(dcFab)       dcFab.style.display       = 'none';
    const mainFab = document.querySelector('.fab');
    if(mainFab)     mainFab.style.display     = '';
    closeAll();
    _currentTab = 'home';
    ...
    if(mmView0) mmView0.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    ...
    if(mmView2) mmView2.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    if(modebar)     modebar.style.display     = 'none';
    if(hdr)         hdr.style.display         = '';
    if(dcView)      dcView.classList.remove('active');
    if(cycleView)   cycleView.classList.add('active');
    if(notesView)   notesView.classList.remove('active');
    if(schedView)   schedView.style.display   = 'none';
    if(settingsView)settingsView.classList.remove('active');
    if(profileView) profileView.classList.remove('active');
    ...
    if(_currentTab !== 'cycle') {
      try{ renderCycleSheet(); }catch(e){ console.error('[Vikram] renderCycleSheet error:',e); }
      _applyCycleTopToggle();
      // Push own cycle data to Firebase so partner sees fresh data
      try{ if(window._fbPair) window._fbPair.forceSync(); }catch(e){}
    }
    _currentTab = 'cycle';
  } else if(tab === 'counter') {
    if(mainContent) mainContent.style.display = 'none';
    const mmView3 = document.getElementById('mmView');
    if(mmView3) mmView3.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    if(modebar)     modebar.style.display     = 'none';
    if(hdr)         hdr.style.display         = '';
    if(cycleView)   cycleView.classList.remove('active');
    if(dcView)      dcView.classList.add('active');
    if(notesView)   notesView.classList.remove('active');
    if(schedView)   schedView.style.display   = 'none';
    if(settingsView)settingsView.classList.remove('active');
    if(profileView) profileView.classList.remove('active');
    scheduleActive = false;
    if(dcFab)       dcFab.style.display       = 'flex';
    const mainFab = document.querySelector('.fab');
    if(mainFab)     mainFab.style.display     = 'none';
    closeAll();
    _currentTab = 'counter';
    try{ if(typeof window.dcRender === 'function') window.dcRender(); }catch(e){ console.error('[Vikram] dcRender error:',e); }
  } else if(tab === 'notes') {
    if(_currentTab !== 'notes') _notesReturnTab = _currentTab || 'home';
    if(mainContent) mainContent.style.display = 'none';
    const mmView4 = document.getElementById('mmView');
    if(mmView4) mmView4.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    if(modebar)     modebar.style.display     = 'none';
    if(hdr)         hdr.style.display         = '';
    if(dcView)      dcView.classList.remove('active');
    ...
    if(_currentTab !== 'settings') _settingsReturnTab = _currentTab || 'home';
    if(mainContent) mainContent.style.display = 'none';
    const mmView5 = document.getElementById('mmView');
    if(mmView5) mmView5.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    if(modebar)     modebar.style.display     = 'none';
    ...
    if(mainFab2)    mainFab2.style.display    = 'none';
    ...
    if(typeof renderGcalCard === 'function') renderGcalCard();
    if(typeof renderWidgetThumb === 'function') renderWidgetThumb();
    if(typeof renderWidgetSteps === 'function') renderWidgetSteps();
    if(typeof updateNotifCard === 'function') updateNotifCard();
  } else if(tab === 'profile') {
    if(mainContent) mainContent.style.display = 'none';
    const mmView6 = document.getElementById('mmView');
    if(mmView6) mmView6.style.display = 'none';
    if(upcoming)    upcoming.style.display    = 'none';
    if(modebar)     modebar.style.display     = 'none';
    if(hdr)         hdr.style.display         = '';
    if(dcView)      dcView.classList.remove('active');
    ...
    if(mainFab3)    mainFab3.style.display    = 'none';
    closeAll();
    _currentTab = 'profile';
    ...
    try{
      const p=JSON.parse(localStorage.getItem('vikram_profile')||'{}');
      if(typeof applyProfilePhoto==='function') applyProfilePhoto(p.photo||null);
      if(typeof profileBdayCardUpdate==='function') profileBdayCardUpdate(p.bday||null);
      const disp=document.getElementById('profileBdayDisplay');
      if(disp) disp.textContent=typeof bdayDisplayText==='function'?bdayDisplayText(p.bday||null):'🎂 Add your birthday';
      if(typeof renderProfileShareEvents==='function') renderProfileShareEvents();
      if(typeof renderProfileBirthdays==='function') renderProfileBirthdays();
      try{ if(typeof window.whRenderCard === 'function') window.whRenderCard(); }catch(e){ console.error('[Vikram] whRenderCard error:',e); }
...
(function(){
  ...
  function dcLoad() {
    try { return JSON.parse(localStorage.getItem(DC_KEY)) || []; } catch(e) { return []; }
  }
  function dcSaveAll(arr) { try{ localStorage.setItem(DC_KEY, JSON.stringify(arr)); }catch(e){} }
  ...
  function dcLoadReminderSettings(){
    try{ return Object.assign({enabled:false, time:'20:00'}, JSON.parse(localStorage.getItem(HABIT_REMINDER_KEY)) || {}); }
    catch(e){ return {enabled:false, time:'20:00'}; }
  }
  function dcSaveReminderSettings(s){ try{ localStorage.setItem(HABIT_REMINDER_KEY, JSON.stringify(s)); }catch(e){} }
  ...
  window.dcToggleHabitReminder = function(){
    ...
    if(s.enabled && typeof requestNotifPerm === 'function' && 'Notification' in window && Notification.permission !== 'granted'){
      requestNotifPerm().then(function(){ dcSaveReminderSettings(s); if(typeof renderActivitySheet==='function') renderActivitySheet(); });
      return;
    }
    dcSaveReminderSettings(s);
    if(typeof renderActivitySheet==='function') renderActivitySheet();
  };
  ...
  window.checkHabitReminders = async function(){
    if(typeof notifEnabled !== 'undefined' && !notifEnabled) return;
    if(!('Notification' in window) || Notification.permission !== 'granted') return;
    const s = dcLoadReminderSettings();
    if(!s.enabled) return;
    const arr = dcLoad();
  ...
  function dcSyncDateField(type) {
    ...
    if(type === 'countdown') {
      ...
      if(nameInput) nameInput.placeholder = 'e.g. Birthday, Exam, Trip... (optional)';
    } else {
      label.textContent = 'Start Date';
      input.removeAttribute('min');
      if(nameInput) nameInput.placeholder = 'e.g. No sugar, Morning walk...';
    }
  }
  ...
  window.dcCloseAdd = function(e) {
    if(e && e.target !== document.getElementById('dcModalOverlay')) return;
  ...
  window.dcSave = function() {
    ...
    if(!name){ document.getElementById('dcNameInput').focus(); return; }
    haptic('success');
    const arr = dcLoad();
    if(_dcEditId !== null){
      const c = arr.find(function(x){return x.id===_dcEditId;});
      if(c){ c.name = name; c.type = _dcType; c.startTs = startTs; c.note = note; }
      _dcEditId = null;
    } else {
  ...
  function nextMilestone(days){
    for(let i=0;i<MILESTONES.length;i++){ if(days < MILESTONES[i]) return {prev: i===0?0:MILESTONES[i-1], target: MILESTONES[i]}; }
  ...
  function dcCompletionPct(c, days){
    const history = c.history || [];
    const startDay = new Date(c.startTs); startDay.setHours(0,0,0,0);
    const today = new Date(); today.setHours(0,0,0,0);
    ...
    for(let i=0;i<denom;i++){
      if(set.has(dayKeyFor(cursor))) count++;
      cursor.setDate(cursor.getDate()-1);
    }
  ...
  function dcUpdateStats(arr) {
    ...
    if(ringNum) ringNum.textContent = checkedToday + '/' + habitArr.length;
    if(ringFg){
    ...
    if(heroEl) heroEl.classList.toggle('complete', isComplete);
    if(statusEl){
  ...
  window.dcRender = function() {
    ...
    sorted.forEach(function(c, i){
      ...
      if(c.type === 'countdown'){
        ...
        const target = new Date(c.startTs); target.setHours(0,0,0,0);
        ...
        if(diffDays > 0){
          bigNum = diffDays; unitLbl = 'day'+(diffDays===1?'':'s'); subLbl = 'until <b>'+esc(dateStr)+'</b>';
        } else if(diffDays === 0){
          bigNum = '\uD83C\uDF89'; unitLbl = 'today'; subLbl = 'It\u2019s today!';
        } else {
          bigNum = Math.abs(diffDays); unitLbl = 'day'+(Math.abs(diffDays)===1?'':'s')+' ago'; subLbl = esc(dateStr) + ' has passed';
        }
  ...
  window.dcToggleMenu = function(e, id){
    ...
    document.querySelectorAll('.dc-card-menu.open').forEach(function(m){
      if(m.id !== 'dcMenu'+id) m.classList.remove('open');
    });
  ...
  document.addEventListener('click', function(e){
    if(_dcSuppressNextClick){ _dcSuppressNextClick = false; return; }
  ...
  function dcPositionMenu(id){
    ...
    if(nav){ const r = nav.getBoundingClientRect(); if(r.top < boundary) boundary = r.top; }
    if(fab && fab.style.display !== 'none'){ const r = fab.getBoundingClientRect(); if(r.top < boundary) boundary = r.top; }
  ...
  function dcInitLongPress(){
    document.querySelectorAll('#dcList .dc-card').forEach(function(card){
      card.addEventListener('pointerdown', function(e: any){
        if(e.target.closest('.dc-drag-handle,.dc-icon-btn,.dc-card-menu')) return;
      ...
      card.addEventListener('pointermove', function(e: any){
        if(!_dcLongPress || _dcLongPress.fired) return;
        const dx = Math.abs(e.clientX - _dcLongPress.startX);
        const dy = Math.abs(e.clientY - _dcLongPress.startY);
        if(dx > 8 || dy > 8){ clearTimeout(_dcLongPress.timer); _dcLongPress = null; }
      });
      ['pointerup','pointercancel','pointerleave'].forEach(function(evt){
        card.addEventListener(evt, function(){
          if(_dcLongPress){ clearTimeout(_dcLongPress.timer); _dcLongPress = null; }
        });
      });
  ...
  function dcDragMove(e){
    ...
    while(prev && (!prev.classList.contains('dc-card') || prev.dataset.pinned !== group)) prev = prev.previousElementSibling;
    let next = card.nextElementSibling;
    while(next && (!next.classList.contains('dc-card') || next.dataset.pinned !== group)) next = next.nextElementSibling;
    ...
    if(prev){
      ...
      if(cardMid < pr.top + pr.height/2){ dcDragReposition(e, prev, true); return; }
    }
    if(next){
      const nr = next.getBoundingClientRect();
      if(cardMid > nr.top + nr.height/2){ dcDragReposition(e, next, false); return; }
    }
  }
  ...
  function dcDragReposition(e, sib, insertBeforeSib){
    ...
    if(insertBeforeSib) list.insertBefore(card, sib);
    else list.insertBefore(card, sib.nextElementSibling);
  ...
  function dcDragEnd(){
    ...
    arr.forEach(function(c){ if(reordered.indexOf(c) === -1) reordered.push(c); });
    dcSaveAll(reordered);
    _dcDrag = null;
    dcRender();
  }
  ...
  window.dcCheckin = function(id) {
    const arr = dcLoad(); const c = arr.find(function(x){return x.id===id;});
    if(!c) return;
    const tk = todayKey();
    ...
    if(wasChecked){ c.history = c.history.filter(function(k){return k!==tk;}); }
    else if(c.history.indexOf(tk)===-1){ c.history.push(tk); }
    dcSaveAll(arr); dcRender();
    const sheet = document.getElementById('activitySheet');
    if(sheet && sheet.classList.contains('open')) renderActivitySheet();
  };
  window.dcReset = function(id) {
    dcShowConfirm('Reset this counter to today? Your current streak will start over.', 'Reset', 'var(--tf)', function(){
      ...
      const arr = dcLoad(); const c = arr.find(function(x){return x.id===id;});
      if(!c) return;
      c.startTs = Date.now(); c.lastCheckin = null; c.history = [];
      dcSaveAll(arr); dcRender();
    });
  };
  ...
  window.dcShowConfirm = function(msg, btnLabel, btnColor, cb) {
    ...
    if(!msgEl || !btnEl) { if(cb) cb(); return; }
  ...
  window.dcConfirmYes = function() {
    ...
    const cb = _dcConfirmCb; _dcConfirmCb = null;
    if(cb) cb();
  };
  window.dcConfirmClose = function(e) {
    if(e && e.target !== document.getElementById('dcConfirmOverlay')) return;
    document.getElementById('dcConfirmOverlay').classList.remove('open');
    _dcConfirmCb = null;
  };
  ...
  function dcBuildActivityGrid(weeksToShow){
    ...
    const today = new Date(); today.setHours(0,0,0,0);
    const curSunday = new Date(today); curSunday.setDate(today.getDate() - today.getDay());
    const gridStart = new Date(curSunday); gridStart.setDate(curSunday.getDate() - (weeksToShow-1)*7);
  ...
  function renderActivitySheet(){
    ...
    try{
      ...
      cols.forEach(function(colDays, idx){
        ...
        if(mo !== lastMonth){ monthLabels.push({col:idx, label:MONTH_NAMES[mo]}); lastMonth = mo; }
      });
...
(function(){
  ...
  function whLoad(){ try{ return JSON.parse(localStorage.getItem(WH_KEY)) || {}; }catch(e){ return {}; } }
  function whSaveAll(obj){ try{ localStorage.setItem(WH_KEY, JSON.stringify(obj)); }catch(e){} }
  ...
  window.whRenderCard = function(){
    ...
    if(totalEl) totalEl.textContent = whFmt(whWeekTotal(data)) + 'h this week';
  ...
  window.whEditToday = function(e){
    ...
    row.innerHTML =
      ...
      + '<button class="wh-save-btn" onclick="whSaveToday()">Save</button>';
    setTimeout(function(){ const el = document.getElementById('whTodayInput'); if(el){ el.focus(); el.select(); } }, 50);
  };
})();
