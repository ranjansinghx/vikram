...
(function(){
  ...
  window.vikramSignIn = function(){
    ...
    if(window.firebase && firebase.auth){
      ...
      firebase.auth().signInWithPopup(provider)
        .then(function(r){
          if(typeof window.updateGoogleUI === 'function') window.updateGoogleUI(r.user);
          if(typeof vikramCloudSync !== 'undefined') vikramCloudSync.startListening(r.user.uid);
      ...
      if(btn) btn.innerHTML = '<span style="font-size:13px;opacity:.7;">Connecting…</span>';
      var attempts = 0;
      var retry = setInterval(function(){
        ...
        if(window.firebase && firebase.auth){
          ...
          if(typeof vikramToast==='function') vikramToast('Ready — tap Continue with Google again');
        } else if(attempts > 20){
          clearInterval(retry);
          if(btn) btn.innerHTML = orig;
          if(typeof vikramToast==='function') vikramToast('❌ Sign-in service unavailable. Try again.');
        }
      }, 500);
    }
  };
  ...
  window._vikramFirebaseReady = function(){
    ...
    if(typeof window.renderPairSection === 'function'){
      try{ window.renderPairSection(); }catch(e){}
    }
