...
(function(){
  ...
  if(tg) tg.classList.toggle('on', window._hapticEnabled);

  // Patch haptic to respect the setting
  const _origHaptic = window.haptic;
  window.haptic = function(type){
    if(!window._hapticEnabled) return;
...
window.togHaptic = function(btn){
  ...
  try{ localStorage.setItem('vikram_haptic', window._hapticEnabled ? '1' : '0'); }catch(e){}
  if(window._hapticEnabled) navigator.vibrate && navigator.vibrate(18);
};
...
(function(){
  ...
  if(cfg.tithi   === undefined) cfg.tithi   = false;
  if(cfg.weekNum === undefined) cfg.weekNum = false;
  if(cfg.compact === undefined) cfg.compact = false;
  if(cfg.hlwknd  === undefined) cfg.hlwknd  = true;
...
window.render = function(){
  if(typeof _origRender2 === 'function') _origRender2.apply(this, arguments);
};
...
window.svConfirmOk = function(){
  document.getElementById('svConfirmModal').style.display = 'none';
  if(_svConfirmCallback) _svConfirmCallback();
  _svConfirmCallback = null;
};
...
window.importEvents = function(input){
  ...
  reader.onload = function(e){
    try{
      ...
      if(!evts || !evts.length){ vikramToast && vikramToast('⚠️ No events found in file'); return; }
      svShowConfirm('📥','Import Events',
        `Add ${evts.length} event${evts.length>1?'s':''} from this file? Existing events are kept.`,
...
window.confirmClearEvents = function(){
  ...
  if(!cnt){ vikramToast && vikramToast('ℹ️ No events to clear'); return; }
  svShowConfirm('🗑️','Clear All Events',
    `This will permanently delete all ${cnt} event${cnt>1?'s':''}. This cannot be undone.`,
    'Delete All', 'linear-gradient(135deg,#e53e3e,#c53030)',
    function(){
...
window.confirmResetApp = function(){
  haptic('light');
  svShowConfirm('⚠️','Reset App Data',
    'This will erase ALL data: events, notes, tasks, settings, profile, and cycle data. This cannot be undone.',
    'Reset Everything', 'linear-gradient(135deg,#7f1d1d,#991b1b)',
    function(){
      // Keep only the installed flag so install prompt doesn't spam
      const installed = localStorage.getItem('vikram_installed');
      localStorage.clear();
      if(installed) localStorage.setItem('vikram_installed', installed);
      haptic('success');
...
window.confirmClearNotes = function(){
  ...
  let cnt = 0;
  try{ const parsed = JSON.parse(raw); cnt = Array.isArray(parsed) ? parsed.length : 0; }catch(e){ cnt = raw.length > 2 ? 1 : 0; }
  if(!cnt){ vikramToast && vikramToast('ℹ️ No notes to clear'); return; }
  svShowConfirm('📝','Clear All Notes',
    `This will permanently delete all ${cnt} note${cnt>1?'s':''}. This cannot be undone.`,
    'Delete All', 'linear-gradient(135deg,#e53e3e,#c53030)',
    function(){
...
window.confirmClearTasks = function(){
  haptic('light');
  let cnt = 0;
  try{ cnt = JSON.parse(localStorage.getItem('vikram_tasks_v1')||'[]').length; }catch(e){}
  if(!cnt){ vikramToast && vikramToast('ℹ️ No tasks to clear'); return; }
  svShowConfirm('✅','Clear All Tasks',
    `This will permanently delete all ${cnt} task${cnt>1?'s':''}. This cannot be undone.`,
    'Delete All', 'linear-gradient(135deg,#e53e3e,#c53030)',
    function(){
...
window.openChangelog = function(){
  ...
  if(ov){ ov.classList.add('open'); ov.dataset.sheetOrigin = 'changelogSheet'; }
};
...
window.openPrivacyPolicy = function(){
  ...
  sheet.classList.add('open');
  const ov = document.getElementById('ov');
  if(ov){ ov.classList.add('open'); ov.dataset.sheetOrigin = 'privacySheet'; }
};
...
function svCalcStorage(){
  ...
  if(other>0){ groupTotals['Other']= other; groupColors['Other']='#9ca3af'; }
...
function svRefreshStorage(){
  ...
  if(sub) sub.textContent = `${fmt(total)} used of ~5 MB`;
...
(function(){
  ...
  window.switchTab = function(tab){
    if(typeof _base === 'function') _base.apply(this, arguments);
    if(tab === 'settings'){
      // Sync profile row
      try{
        ...
        if(subEl){
          ...
          } else { subEl.textContent = 'Edit profile, birthday & more'; }
        }
      }catch(e){}
      // Sync haptic
