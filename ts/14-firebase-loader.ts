(function(){
  ...
  function loadFirebase(){
    ...
    function loadNext(){
      ...
      s.async=true;
      s.onload=function(){
        if(loaded<scripts.length) loadNext();
        else initFirebaseApp();
      };
      s.onerror=function(){ /* silently skip if offline */ };
  ...
  function initFirebaseApp(){
    ...
    try {
      ...
      if(window._fbPair){
        ...
        setTimeout(function(){
          if(typeof window.renderPairSection === 'function') window.renderPairSection();
        }, 0);
        // Also re-push after a short delay in case RTDB connection takes a moment to establish
        setTimeout(function(){
          if(window._fbPair) window._fbPair.forceSync();
        }, 2000);
      }
      ...
      if(firebase.auth){
        function updateGoogleUI(user){
          ...
          if(user && !user.isAnonymous){
            ...
            var av=document.getElementById('googleAvatar'); if(av) av.src=user.photoURL||'';
            var dn=document.getElementById('googleDisplayName'); if(dn) dn.textContent=user.displayName||'';
            var em=document.getElementById('googleEmail'); if(em) em.textContent=user.email||'';
            var ha=document.getElementById('hdrProfileAvatar');
            if(ha&&user.photoURL){ha.innerHTML='';ha.style.background='none';var img=document.createElement('img');img.src=user.photoURL;img.style.cssText='width:16px;height:16px;border-radius:50%;object-fit:cover;';ha.appendChild(img);}
          } else {
            btn.style.display='flex'; info.style.display='none';
            var ha=document.getElementById('hdrProfileAvatar');
        ...
        window.signInWithGoogle=function(){
          ...
          firebase.auth().signInWithPopup(provider)
            .then(function(r){
              ...
              if(typeof vikramToast==='function') vikramToast('✅ Signed in as '+(r.user.displayName||r.user.email));
            })
            .catch(function(e){
              console.warn('signInWithPopup failed:', e.code, e.message);
              if(typeof vikramToast==='function') vikramToast('❌ Sign-in failed: '+e.code);
            });
        };
        window.signOutGoogle=function(){
          if(typeof haptic==='function') haptic('light');
          firebase.auth().signOut().then(function(){updateGoogleUI(null);if(typeof vikramToast==='function')vikramToast('👋 Signed out');});
        };
      ...
      if(typeof window._vikramFirebaseReady === 'function') window._vikramFirebaseReady();
    } catch(e){ console.warn('[Vikram] Firebase init failed:',e); }
  }
